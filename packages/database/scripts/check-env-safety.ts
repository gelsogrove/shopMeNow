 ...
 */
export function isDestructiveOperationsAllowed(): boolean {
 ...
 */
export function ensureNotProduction(scriptName = "this script"): void {
  ...
  if (isProduction) {
    if (allowBypass) {
      console.log(
        `${YELLOW}${BOLD}⚠️  WARNING: Production environment detected but ALLOW_DESTRUCTIVE_OPERATIONS=true${RESET}`
      )
 ...
 */
export const DESTRUCTIVE_SCRIPTS = [
  "prisma:seed",
  "prisma:reset",
  "migrate:reset",
  "db:push",
