...
const check_env_safety_1 = require("../scripts/check-env-safety");
(0, check_env_safety_1.ensureNotProduction)("prisma:seed");
const client_1 = require("../src/generated/prisma/client");
const adapter_pg_1 = require("@prisma/adapter-pg");
const bcrypt = __importStar(require("bcrypt"));
const campaigns_1 = require("./data/campaigns");
const categories_1 = require("./data/categories");
const defaultAgents_1 = require("./data/defaultAgents");
const defaultFAQs_1 = require("./data/defaultFAQs");
const faqs_1 = require("./data/faqs");
const offers_1 = require("./data/offers");
const platformConfig_1 = require("./data/platformConfig");
const pricingConfig_1 = require("./data/pricingConfig");
...
Sono qui per aiutarti 😊`,
            ...
            humanSupportInstructions: "Ciao {{nameUser}}, mi sto mettendo in contatto con l'agente {{agentName}}. Ti richiamera' al piu' presto (tel: {{agentPhone}} - email: {{agentEmail}}). Disattivo il chatbot finche' non ricevi risposta.",
...
- Per tutti gli altri casi, rispondi normalmente senza escalation`,
    ...
    const infoWorkspace = await prisma.workspace.create({
        data: {
            ...
            hasHumanSupport: true,
            humanSupportInstructions: "Ciao {{nameUser}}, mi sto mettendo in contatto con il nostro operatore. Ti rispondera' al piu' presto. Disattivo il chatbot finche' non ricevi assistenza.",
...
⚠️ IMPORTANTE: Prima verifica SEMPRE se la risposta è nelle FAQ o nella knowledge base`,
    ...
    console.log(`✅ Created ${createdInfoCustomers.length} informational customers with multilingual profiles`);
    ...
    const infoChatSamples = [
        {
            ...
            assistant: "Ciao! Per i prodotti freschi usiamo trasporto refrigerato dedicato con consegne 48h in tutta Italia.",
            history: [
                {
                    role: "user",
                ...
                {
                    role: "assistant",
                    content: "Sì, i mezzi refrigerati partono ogni lunedì e giovedì da Barcellona verso il Nord Italia.",
                    minutesAgo: 179,
                },
            ],
        },
        {
            ...
            assistant: "Yes, every parcel includes a tracking link so you can follow the delivery in real time.",
            history: [
                {
                    role: "user",
                ...
                {
                    role: "assistant",
                    content: "Of course, we can drop-ship samples or brochures directly to your customer with neutral packaging.",
                    minutesAgo: 199,
                },
            ],
        },
        {
            language: "ESP",
            inbound: "Hola, necesito información sobre envíos combinados de ambiente y refrigerado.",
            assistant: "Podemos preparar pedidos mixtos: agrupamos los productos ambiente y enviamos la parte refrigerada en un segundo pallet.",
            history: [
                {
                    role: "user",
                ...
                {
                    role: "assistant",
                    content: "Sí, podemos coordinar franjas de mañana o tarde con nuestros transportistas locales.",
                    minutesAgo: 164,
                },
            ],
        },
        {
            ...
            assistant: "Olá! Trabalhamos com camiões frigoríficos certificados e entregamos em Portugal continental duas vezes por semana.",
            history: [
                {
                    role: "user",
                    content: "Conseguem entregar diretamente aos meus restaurantes com prova de entrega?",
                    minutesAgo: 140,
                },
                {
                    role: "assistant",
                    content: "Sim, fornecemos POD digital assinada e podemos enviar a cópia por e-mail assim que o motorista conclui a entrega.",
                    minutesAgo: 139,
                },
            ],
        },
    ...
    for (const chatData of infoChatSamples) {
        const customer = createdInfoCustomers.find((c) => c.template.language === chatData.language);
        if (!customer) {
        ...
        await prisma.conversationMessage.createMany({
            data: chatData.history.map((entry, index) => ({
                ...
                createdAt: new Date(Date.now() - 1000 * 60 * (entry.minutesAgo ?? 60) + index * 500),
            })),
        });
    }
    ...
    const ecommerceAgents = (0, defaultAgents_1.defaultAgents)(ecommerceWorkspace.id);
    ...
    const infoAgents = (0, defaultAgents_1.defaultAgents)(infoWorkspace.id).filter((agent) => !ECOMMERCE_ONLY_TYPES.includes(agent.type));
    ...
    const supportWorkspace = await prisma.workspace.create({
        data: {
            ...
            channelStatus: true,
            welcomeMessage: "Hi! I'm your eChatbot product assistant. Ask me anything about plans, integrations or onboarding.",
            wipMessage: "Our assistants are being updated. If you need immediate help please write to support@echatbot.ai.",
            ownerId: adminUser.id,
            planType: "ENTERPRISE",
            creditBalance: 500,
            trialEndsAt: null,
            ...
            hasSalesAgents: false,
            hasHumanSupport: true,
            humanSupportInstructions: "Ciao {{nameUser}}, ti metto subito in contatto con un consulente eChatbot. Riceverai risposta entro 15 minuti da {{agentName}} (tel: {{agentPhone}} / email: {{agentEmail}}).",
...
⚠️ Prima verifica SEMPRE nella documentazione/FAQ se esiste già una risposta`,
            operatorContactMethod: "EMAIL",
            toneOfVoice: "PROFESSIONAL",
            botIdentityResponse: "I'm the eChatbot product specialist. I can explain pricing, automation features, integrations and connect you with a human consultant anytime.",
        },
    });
    console.log(`✅ Enterprise support workspace created: ${supportWorkspace.name} (${supportWorkspace.id})`);
    await prisma.userWorkspace.create({
        data: {
            userId: adminUser.id,
    ...
    const supportAgents = (0, defaultAgents_1.defaultAgents)(supportWorkspace.id).filter((agent) => !["PRODUCT_SEARCH", "CART_MANAGEMENT", "ORDER_TRACKING"].includes(agent.type));
    ...
    const supportFAQs = [
        {
            ...
            answer: "We offer Starter, Premium and Enterprise plans. Pricing is usage-based: a monthly platform fee plus credits for WhatsApp conversations, automations and AI tasks. Enterprise includes custom SLAs and concierge onboarding.",
        },
        {
            category: "Pricing",
            question: "Can I test eChatbot before committing to a paid plan?",
            answer: "Yes, every workspace starts with a fully-featured trial environment so you can validate flows, integrations and analytics. No card is required during the trial.",
        },
        {
            category: "Platform",
            question: "Which channels are supported besides WhatsApp?",
            answer: "WhatsApp is the primary channel, but eChatbot also integrates email, Instagram DM, web chat widgets and backoffice messaging via the same unified automation stack.",
        },
        {
            category: "Platform",
            question: "Does eChatbot support multilingual conversations?",
            answer: "Absolutely. Each workspace can configure Italian, English, Spanish, Portuguese and more. The AI router translates messages while keeping tone, brand voice and compliance consistent.",
        },
        {
            category: "Integrations",
            question: "How do I connect eChatbot with my ERP or CRM?",
            answer: "We provide REST APIs, webhooks, native connectors for HubSpot/Shopify, and middleware guides for custom ERPs. Enterprise clients receive solution architect support during integration.",
        },
        {
            category: "Support",
            question: "Can I talk with a human consultant if the bot can’t solve my issue?",
            answer: "Yes. Every plan includes human hand-off. Just request an agent and our team opens an email/WhatsApp thread with a named consultant who replies within SLA.",
        },
        {
            category: "Security",
            question: "Where is customer data hosted and how is it protected?",
            answer: "Data is hosted in the EU on AWS, encrypted at rest and in transit. We follow multi-tenant isolation, role-based access control and audit logging aligned with GDPR requirements.",
        },
        {
            category: "Billing",
            question: "Which payment methods do you accept for subscriptions?",
            answer: "We accept SEPA transfer, credit cards (Visa/Mastercard) and invoicing with 30-day terms for Enterprise customers.",
        },
        {
            category: "Automation",
            question: "Can eChatbot escalate complex workflows to my internal team?",
            answer: "Yes. The workflow engine can trigger Zendesk/Asana tasks, send Slack alerts or forward transcripts via email so your team continues the conversation seamlessly.",
        },
        {
            category: "Analytics",
            question: "What type of analytics can I expect in the dashboard?",
            answer: "You can monitor conversation volume, conversion funnels, agent performance, translation usage, CSAT trends and export raw data for BI tools.",
        },
        {
            category: "Compliance",
            question: "Is eChatbot Meta BSP compliant and do you manage WhatsApp templates?",
            answer: "Yes, we are aligned with Meta BSP guidelines. We help you register templates, manage quality scores and automate template fallback logic.",
        },
        {
            category: "Deployment",
        ...
        },
        {
            ...
            answer: "Certainly. You can plug in your own OpenAI/Anthropic keys, set model policies per agent and even mix internal deterministic flows with LLMs.",
        },
        {
            category: "Team",
            question: "How many operators can collaborate in the console?",
            answer: "There’s no hard cap. Role-based permissions let you add unlimited operators, admins and analysts while tracking their actions in the audit log.",
        },
        {
            category: "Roadmap",
            question: "Do you support custom roadmaps or feature requests?",
            answer: "Enterprise contracts include roadmap alignment sessions and private betas. We prioritize features that align with your automation KPIs.",
        },
    ];
    ...
    await prisma.message.create({
        data: {
            chatSessionId: supportSession.id,
            direction: "OUTBOUND",
            content: "Hi Sara! Enterprise starts from €899/month and scales with active workspaces. I can share a tailored quote or connect you with a consultant.",
            type: "TEXT",
            aiGenerated: true,
            metadata: {
                agentSelected: "CUSTOMER_SUPPORT",
        ...
        data: [
            {
                ...
                customerId: supportProspect.id,
                role: "user",
                content: "Great! I'd like a live walkthrough and to involve my operations team.",
                createdAt: new Date(Date.now() - 1000 * 60 * 2),
            },
            {
                conversationId: supportSession.id,
                ...
                customerId: supportProspect.id,
                role: "assistant",
                content: "Perfect, I'm escalating this to our human operator. You'll receive a calendar invitation and WhatsApp follow-up within 15 minutes.",
    ...
    console.log(`   - Channels: 3 (E-commerce: BellItalia VIP, Informational: BellItalia, Support: eChatbot HQ)\n`);
    ...
    for (const pricing of pricingConfig_1.pricingConfigData) {
    ...
    console.log(`✅ Created/Updated ${pricingConfig_1.pricingConfigData.length} pricing configurations`);
    console.log(`   - Plans: ${pricingConfig_1.pricingConfigData.filter((p) => p.type === "PLAN").length}`);
    console.log(`   - Usage: ${pricingConfig_1.pricingConfigData.filter((p) => p.type === "USAGE").length}`);
    console.log(`   - Thresholds: ${pricingConfig_1.pricingConfigData.filter((p) => p.type === "THRESHOLD").length}`);
    // 6b. Create Platform Configuration (NEW Single Source of Truth)
    console.log("🚀 Creating platform configuration (NEW)...");
    for (const config of platformConfig_1.platformConfigData) {
    ...
    console.log(`✅ Created/Updated ${platformConfig_1.platformConfigData.length} platform configurations`);
    console.log(`   - Prices: ${platformConfig_1.platformConfigData.filter((p) => p.type === "PRICE").length}`);
    console.log(`   - Flags: ${platformConfig_1.platformConfigData.filter((p) => p.type === "FLAG").length}`);
    console.log(`   - Limits: ${platformConfig_1.platformConfigData.filter((p) => p.type === "LIMIT").length}`);
    // 7. Create Categories
    ...
    console.log(`✅ Created ${categories_1.categories.length} categories`);
    ...
    for (const prod of products_1.products) {
        ...
        const isVegan = prod.name.toLowerCase().includes("vegan") ||
            prod.name.toLowerCase().includes("vegano");
        const isGlutenFree = prod.name.toLowerCase().includes("gluten-free") ||
        ...
        const isWholeGrain = prod.name.toLowerCase().includes("whole") ||
        ...
        const isDOP = prod.name.toLowerCase().includes("dop") ||
        ...
        const certificationNames = [];
        if (isOrganic)
            certificationNames.push("Bio");
        if (isVegan)
            certificationNames.push("Vegan");
        if (isGlutenFree)
            certificationNames.push("Gluten-Free");
        if (isHalal)
            certificationNames.push("Halal");
        if (isWholeGrain)
            certificationNames.push("Whole-Grain");
        if (isDOP)
            certificationNames.push("DOP");
    ...
    console.log(`✅ Created ${products_1.products.length} products`);
    // 8. Create Services
    console.log("🛠️  Creating services...");
    for (const svc of services_1.services) {
    ...
    console.log(`✅ Created ${services_1.services.length} services`);
    // 9. Create Offers
    console.log("🎯 Creating offers...");
    // Get first category as fallback
    const firstCategoryId = Array.from(categoryMap.values())[0];
    for (const offer of offers_1.offers) {
        // Find category by name from export data
        let categoryId = firstCategoryId; // Default fallback
        if (offer.categoryName) {
            const foundId = categoryMap.get(offer.categoryName);
            if (foundId) {
                categoryId = foundId;
                console.log(`  ✅ Offer "${offer.name}" → Category "${offer.categoryName}"`);
            }
            else {
                console.log(`  ⚠️  Category "${offer.categoryName}" not found, using fallback`);
            }
        }
    ...
    console.log(`✅ Created ${offers_1.offers.length} offers`);
    // 10. Create FAQs (from defaultFAQs with keywords + category)
    console.log("❓ Creating FAQs...");
    const defaultFaqList = (0, defaultFAQs_1.defaultFAQs)(workspace.id);
    ...
    for (const campaign of campaigns_1.campaigns) {
    ...
    console.log(`✅ Created ${campaigns_1.campaigns.length} campaigns`);
    ...
    console.log(`✅ Created ${testCustomers.length} test customers (distributed Apr-Jul 2025)`);
    console.log(`   📱 First 2 (Mario, João): active, push consent, GDPR accepted → eligible for campaigns`);
    console.log(`   🚫 Last 2 (Maria, John): blacklisted, no consent → NOT eligible for campaigns`);
    ...
    const italianCustomer = allCustomers.find((c) => c.language === "IT" || c.language === "it");
    if (italianCustomer) {
        console.log(`   Creating chat for: ${italianCustomer.name} (${italianCustomer.language})`);
        ...
        await prisma.conversationMessage.createMany({
            data: [
                ...
                },
                {
                    ...
                    customerId: italianCustomer.id,
                    role: "assistant",
                    content: "✅ Perfetto! Ho aggiunto al carrello:\n\n**Parmigiano Reggiano DOP**\nQuantità: 1 kg\nPrezzo: €18.50\n\nVuoi procedere con l'ordine?",
                    debugInfo: JSON.stringify({
                        agentSelected: "CART",
    ...
    const spanishCustomer = allCustomers.find((c) => c.language === "ESP" || c.language === "es");
    if (spanishCustomer) {
        console.log(`   Creating chat for: ${spanishCustomer.name} (${spanishCustomer.language})`);
        ...
        await prisma.conversationMessage.createMany({
            data: [
                ...
                },
                {
                    ...
                    customerId: spanishCustomer.id,
                    role: "assistant",
                    content: "¡Hola! Tu pedido está en tránsito 🚚\n\n📦 **Pedido #ORD-001**\nEstado: En camino\nEntrega estimada: Mañana\n\n¿Necesitas algo más?",
                    debugInfo: JSON.stringify({
    ...
    const portugueseCustomer = allCustomers.find((c) => c.language === "PRT" || c.language === "pt");
    if (portugueseCustomer) {
        console.log(`   Creating chat for: ${portugueseCustomer.name} (${portugueseCustomer.language})`);
        ...
        await prisma.conversationMessage.createMany({
            data: [
                ...
                },
                {
                    ...
                    customerId: portugueseCustomer.id,
                    role: "assistant",
                    content: "Olá! Sim, temos ofertas especiais:\n\n🎉 **Oferta da Semana**:\n- Azeite Toscano: -20%\n- Prosciutto di Parma: -15%\n\nQuer aproveitar?",
                    debugInfo: JSON.stringify({
                        agentSelected: "PRODUCT_SEARCH",
                    }),
    ...
    const englishCustomer = allCustomers.find((c) => c.language === "ENG" || c.language === "en");
    if (englishCustomer) {
        console.log(`   Creating chat for: ${englishCustomer.name} (${englishCustomer.language})`);
    ...
    if (productsList.length > 0 && customersList.length > 0) {
        ...
        async function createOrder(orderCode, customerId, status, date, productIds, quantities) {
        ...
        for (const { month, year, days } of months) {
            for (const day of days) {
                ...
                await createOrder(`ORD-${String(orderCount).padStart(3, "0")}-${year}-${month}`, customer.id, status, new Date(year, month - 1, day), selectedProducts, quantities);
                orderCount++;
            }
        }
        console.log(`✅ Created ${orderCount - 1} historical orders with sales records`);
    }
    else {
        console.log("⚠️  No products or customers found, skipping historical orders");
    }
    ...
    if (productsList.length > 0 && customersList.length > 0) {
        ...
        console.log(`   - Top searched: ${topProducts[0].name} (${topProducts[0].searches} searches)`);
        console.log(`   - Distribution: Last 30 days with realistic hourly patterns`);
    }
    else {
    ...
    const messageCost = pricingConfig_1.pricingConfigData.find((p) => p.key === "MESSAGE")?.value ?? 0.15;
    ...
    if (customersForMessages.length > 0) {
        ...
        for (const monthData of monthsToSimulate) {
            for (let i = 0; i < customersForMessages.length &&
                i < monthData.messagesPerCustomer.length; i++) {
    ...
    const monthlyChannelCost = pricingConfig_1.pricingConfigData.find((p) => p.key === "MONTHLY_CHANNEL_COST")?.value ?? 49;
    ...
    const pushCampaignCost = pricingConfig_1.pricingConfigData.find((p) => p.key === "PUSH_CAMPAIGN")?.value ?? 1.0;
    ...
    console.log(`   - Categories: ${categories_1.categories.length}`);
    console.log(`   - Products: ${products_1.products.length}`);
    console.log(`   - Services: ${services_1.services.length}`);
    console.log(`   - Offers: ${offers_1.offers.length}`);
    console.log(`   - FAQs: ${faqs_1.faqs.length}`);
    console.log(`   - Campaigns: ${campaigns_1.campaigns.length}`);
