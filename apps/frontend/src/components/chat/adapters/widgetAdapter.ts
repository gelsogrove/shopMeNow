...
export const sendWidgetMessage = async ({
  ...
  return {
    ...
    // 🔬 debugMode only: the guest's stay-state after this turn, for the
    // widget's live state strip.
    debugState: data.debugState as Record<string, unknown> | undefined,
...
export const registerAndStartChat = async (input: WidgetRegisterInput) => {
  const { apiUrl, workspaceId, visitorId, name, phone, language, firstMessage, pushNotificationsConsent } = input
