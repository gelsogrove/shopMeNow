...
import { TouristChurchFormFields } from "@/components/tourist/TouristChurchFormFields"
...
import { TouristChurch, touristChurchApi } from "@/services/touristChurchApi"

export function TouristChurchesPage() {
  const navigate = useNavigate()
  const { workspace, loading: isLoadingWorkspace } = useWorkspace()
  const [items, setItems] = useState<TouristChurch[]>([])
  const [isLoading, setIsLoading] = useState(true)
  const [currentPage, setCurrentPage] = useState(1)
  const [showAddSheet, setShowAddSheet] = useState(false)
  const [showEditSheet, setShowEditSheet] = useState(false)
  const [showDeleteDialog, setShowDeleteDialog] = useState(false)
  const [selectedItem, setSelectedItem] = useState<TouristChurch | null>(null)
  ...
  const loadItems = async () => {
    ...
    try {
      const data = await touristChurchApi.getTouristChurches(workspace.id)
      setItems(data)
    } catch (error) {
      logger.error("Error loading tourist-churches:", error)
      toast.error("Failed to load chiese")
    } finally {
      setIsLoading(false)
    }
  }
  ...
  const handleAdd = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const created = await touristChurchApi.createTouristChurch(workspace.id, data)
  ...
  const handleEdit = (item: TouristChurch) => {
    setSelectedItem(item)
    setShowEditSheet(true)
  }
  ...
  const handleEditSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const data = readFormData(e.target as HTMLFormElement)
    try {
      const updated = await touristChurchApi.updateTouristChurch(
        workspace.id,
        selectedItem.id,
        data
      )
  ...
  const handleDelete = (item: TouristChurch) => {
    setSelectedItem(item)
  ...
  const handleDeleteConfirm = async () => {
    if (!selectedItem || !workspace?.id) return
    try {
      await touristChurchApi.deleteTouristChurch(workspace.id, selectedItem.id)
  ...
  return (
    <PageLayout>
      ...
      <LargeFormDialog
        ...
        onSubmit={handleAdd}
      >
        <TouristChurchFormFields item={null} workspaceId={workspace.id} />
      </LargeFormDialog>
      ...
      >
        {selectedItem && (
          <TouristChurchFormFields item={selectedItem} workspaceId={workspace.id} />
        )}
      </LargeFormDialog>
