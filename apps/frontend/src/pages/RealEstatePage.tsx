...
import {
  ...
  type RealEstateCopy,
  type RealEstateLang,
} from "./real-estate/real-estate.i18n"
...
export function RealEstatePage() {
  ...
  const t = REAL_ESTATE_I18N[(language as RealEstateLang) ?? "en"] ?? REAL_ESTATE_I18N.en
  ...
  return (
    <>
      <SEO
        ...
        keywords={t.seoKeys}
        url="/real-estate"
        lang={language as RealEstateLang}
        serviceType="Real Estate Agency WhatsApp Chatbot"
      />
      <SiteHeader />
      ...
      <main className="bg-[#070d18] text-slate-200">
        ...
        <section className="border-y border-white/5 bg-white/[0.02] py-16 lg:py-24">
          <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
            <SectionHeading title={t.loaderTitle} accent={t.loaderAccent} sub={t.loaderSub} />
            <PropertyLoader t={t} />
          </div>
        </section>
 ...
 *  The Arabic stays fixed (the foreign buyer); every other line follows the
 *  active UI language so the visual never contradicts the page copy. */
function PhoneChat({ t }: { t: RealEstateCopy }) {
  return (
          ...
          <div className="flex items-center gap-3 bg-[#075E54] px-4 py-3 text-white">
            ...
            <div className="leading-tight">
              ...
              <p className="flex items-center gap-1 text-[11px] text-white/70">
                <MapPin className="h-3 w-3" /> DemoCasa · online
              </p>
            </div>
          </div>
          <div className="min-h-[260px] space-y-3 px-3 py-4 text-[13px]">
            <div className="flex justify-start">
              <div className="max-w-[80%] rounded-2xl rounded-tl-sm bg-white px-3 py-2 shadow-sm" dir="rtl">
                <p className="text-gray-900">هل الشقة في وسط المدينة ما زالت متاحة؟</p>
            ...
            <div className="flex justify-start">
              <div className="max-w-[80%] rounded-2xl rounded-tl-sm bg-white px-3 py-2 shadow-sm" dir="rtl">
                <p className="text-gray-900">شكراً! هل يمكنني زيارتها؟</p>
                <p className="mt-1 border-t border-gray-100 pt-1 text-[11px] italic text-gray-400" dir="ltr">
...
/** WhatsApp phone mock: AI books a viewing + upsells mortgage consultation. */
function ActsChat({ t }: { t: RealEstateCopy }) {
  return (
...
function PushMock({ t }: { t: RealEstateCopy }) {
  return (
    <motion.div {...reveal} className="mx-auto w-full max-w-md">
...
// ---------------------------------------------------------------------------
// Animated per-property data loader — cycles through listings and "loads" each
// property's price, surface and rooms, so the visitor SEES that the AI injects
// the right data per listing.
// ---------------------------------------------------------------------------
// Country-neutral example data — labelled by listing REFERENCE (not specific
// addresses), so the same demo fits any real estate agency anywhere.
...
const PROPERTY_DATA: PropertyData[] = [
  { price: "€245.000", surface: "85 m²", rooms: "3" },
  { price: "€420.000", surface: "120 m²", rooms: "4" },
  { price: "€180.000", surface: "60 m²", rooms: "2" },
  { price: "€650.000", surface: "150 m²", rooms: "5" },
]

function PropertyLoader({ t }: { t: RealEstateCopy }) {
  const [active, setActive] = useState(0)
  const listings = t.listings

  useEffect(() => {
    const id = setInterval(() => setActive((a) => (a + 1) % PROPERTY_DATA.length), 3600)
    return () => clearInterval(id)
  }, [])

  const s = PROPERTY_DATA[active]
  ...
  return (
    ...
    >
      ...
      <div className="relative overflow-hidden rounded-2xl border border-white/10 bg-slate-900/50 p-6 lg:p-8">
        {/* Loading progress bar (re-runs on every listing switch) */}
        <motion.div
          key={`bar-${active}`}
          initial={{ width: "0%" }}
          animate={{ width: "100%" }}
        ...
        <AnimatePresence mode="wait">
          ...
          >
            ...
            >
              <MapPin className="h-5 w-5 text-green-400" /> {listings[active]}
            </motion.h3>
            ...
            <div className="space-y-3">
              ...
              >
                ...
                <span className="text-sm text-slate-400">{t.priceLabel}</span>
                <span className="ml-auto font-mono font-semibold text-green-300">{s.price}</span>
              </motion.div>
              ...
              <motion.div
                ...
                className="flex items-center gap-3 rounded-xl border border-white/10 bg-white/[0.03] px-4 py-3"
              >
                <Ruler className="h-5 w-5 shrink-0 text-green-400" />
                <span className="text-sm text-slate-400">{t.surfaceLabel}</span>
                <span className="ml-auto font-mono font-semibold text-white">{s.surface}</span>
              </motion.div>
              ...
              <motion.div
                ...
                className="flex items-center gap-3 rounded-xl border border-white/10 bg-white/[0.03] px-4 py-3"
              >
                <DoorOpen className="h-5 w-5 shrink-0 text-green-400" />
                <span className="text-sm text-slate-400">{t.roomsLabel}</span>
                <span className="ml-auto font-mono font-semibold text-white">{s.rooms}</span>
              </motion.div>
            </div>
          </motion.div>
        </AnimatePresence>
