...
export function TouristEventsPage() {
  ...
  const [selectedItem, setSelectedItem] = useState<TouristEvent | null>(null)
  ...
  const loadEvents = async () => {
    ...
    try {
      const data = await touristEventApi.getTouristEvents(workspace.id)
      setEvents(data)
    } catch (error) {
      logger.error("Error loading tourist events:", error)
  ...
  useEffect(() => {
    if (!isLoadingWorkspace) loadEvents()
  ...
  useEffect(() => {
    ...
    const item = events.find((ev) => ev.id === editId)
    if (item) {
  ...
  const readFormData = (form: HTMLFormElement) => {
    ...
    return {
      ...
      ticketInfo: (formData.get("ticketInfo") as string) || null,
  ...
  const handleAdd = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const created = await touristEventApi.createTouristEvent(workspace.id, data)
  ...
  const handleEdit = (item: TouristEvent) => {
    setSelectedItem(item)
    setShowEditSheet(true)
  }
  ...
  const handleEditSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const data = readFormData(e.target as HTMLFormElement)
    try {
      const updated = await touristEventApi.updateTouristEvent(
        workspace.id,
        selectedItem.id,
        data
      )
      setEvents(events.map((ev) => (ev.id === selectedItem.id ? updated : ev)))
  ...
  const handleDelete = (item: TouristEvent) => {
    setSelectedItem(item)
  ...
  const handleDeleteConfirm = async () => {
    if (!selectedItem || !workspace?.id) return
    try {
      await touristEventApi.deleteTouristEvent(workspace.id, selectedItem.id)
      setEvents(events.filter((ev) => ev.id !== selectedItem.id))
  ...
  return (
    <PageLayout>
      ...
      <FormSheet
        ...
        title="Add Event"
        description="Add a new event shown to customers by the chatbot."
        onSubmit={handleAdd}
      >
      ...
      <FormSheet
        ...
        title="Edit Event"
        description="Update this event's details."
