...
import { TouristVenueFormFields } from "@/components/tourist/TouristVenueFormFields"
...
import { TouristVenue, touristVenueApi } from "@/services/touristVenueApi"

export function TouristVenuesPage() {
  const navigate = useNavigate()
  const { workspace, loading: isLoadingWorkspace } = useWorkspace()
  const [items, setItems] = useState<TouristVenue[]>([])
  const [isLoading, setIsLoading] = useState(true)
  const [currentPage, setCurrentPage] = useState(1)
  const [showAddSheet, setShowAddSheet] = useState(false)
  const [showEditSheet, setShowEditSheet] = useState(false)
  const [showDeleteDialog, setShowDeleteDialog] = useState(false)
  const [selectedItem, setSelectedItem] = useState<TouristVenue | null>(null)
  ...
  const loadItems = async () => {
    ...
    try {
      const data = await touristVenueApi.getTouristVenues(workspace.id)
      setItems(data)
    } catch (error) {
      logger.error("Error loading tourist-venues:", error)
      toast.error("Failed to load locali")
    } finally {
      setIsLoading(false)
    }
  }
  ...
  const handleAdd = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const created = await touristVenueApi.createTouristVenue(workspace.id, data)
  ...
  const handleEdit = (item: TouristVenue) => {
    setSelectedItem(item)
    setShowEditSheet(true)
  }
  ...
  const handleEditSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const data = readFormData(e.target as HTMLFormElement)
    try {
      const updated = await touristVenueApi.updateTouristVenue(
        workspace.id,
        selectedItem.id,
        data
      )
  ...
  const handleDelete = (item: TouristVenue) => {
    setSelectedItem(item)
  ...
  const handleDeleteConfirm = async () => {
    if (!selectedItem || !workspace?.id) return
    try {
      await touristVenueApi.deleteTouristVenue(workspace.id, selectedItem.id)
  ...
  return (
    <PageLayout>
          ...
          renderContent={(item) => (
            <>
              ...
              <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-gray-500">
                {item.venueType && <span>{item.venueType}</span>}
      ...
      <FormDialog
        ...
        onSubmit={handleAdd}
      >
        <TouristVenueFormFields item={null} workspaceId={workspace.id} />
      </FormDialog>
      ...
      >
        {selectedItem && (
          <TouristVenueFormFields item={selectedItem} workspaceId={workspace.id} />
        )}
      </FormDialog>
