...
import {
  ...
  touristSportsFacilityApi,
} from "@/services/touristSportsFacilityApi"

export function TouristSportsFacilitiesPage() {
  const navigate = useNavigate()
  const { workspace, loading: isLoadingWorkspace } = useWorkspace()
  const [facilities, setFacilities] = useState<TouristSportsFacility[]>([])
  const [isLoading, setIsLoading] = useState(true)
  const [currentPage, setCurrentPage] = useState(1)
  const [showAddSheet, setShowAddSheet] = useState(false)
  const [showEditSheet, setShowEditSheet] = useState(false)
  const [showDeleteDialog, setShowDeleteDialog] = useState(false)
  const [selectedItem, setSelectedItem] = useState<TouristSportsFacility | null>(null)
  ...
  const loadFacilities = async () => {
    ...
    try {
      const data = await touristSportsFacilityApi.getTouristSportsFacilities(
        workspace.id
      )
      setFacilities(data)
    } catch (error) {
      logger.error("Error loading tourist sports facilities:", error)
      toast.error("Failed to load sports facilities")
    } finally {
      setIsLoading(false)
    }
  }
  ...
  const handleAdd = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const created = await touristSportsFacilityApi.createTouristSportsFacility(
        workspace.id,
        data
      )
  ...
  const handleEdit = (item: TouristSportsFacility) => {
    setSelectedItem(item)
    setShowEditSheet(true)
  }
  ...
  const handleEditSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const data = readFormData(e.target as HTMLFormElement)
    try {
      const updated = await touristSportsFacilityApi.updateTouristSportsFacility(
        workspace.id,
        selectedItem.id,
        data
      )
  ...
  const handleDelete = (item: TouristSportsFacility) => {
    setSelectedItem(item)
  ...
  const handleDeleteConfirm = async () => {
    if (!selectedItem || !workspace?.id) return
    try {
      await touristSportsFacilityApi.deleteTouristSportsFacility(
        workspace.id,
        selectedItem.id
      )
  ...
  return (
    <PageLayout>
      ...
      <FormSheet
        ...
        description="Add a new sports facility shown to customers by the chatbot."
        onSubmit={handleAdd}
      >
      ...
      <FormSheet
        ...
        description="Update this sports facility's details."
