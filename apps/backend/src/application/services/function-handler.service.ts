    ...
    try {
      ...
      const orderItems = order.items.map((item, idx) => {
        ...
        return {
          ...
          totalPrice: Number(item.totalPrice) || (item.quantity || 1) * (Number(item.unitPrice) || 0),
          sku: itemCode,
      ...
      const result = {
        ...
        order: {
          ...
          subtotalAmount: Number(order.totalAmount) - Number(order.shippingAmount || 0) - Number(order.taxAmount || 0) + Number(order.discountAmount || 0),
