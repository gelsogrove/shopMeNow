...
export function matchCategory(
  ...
  if (bestMatch) {
    logger.debug(`🔍 Keyword match: CATEGORY "${message}" → "${bestMatch.matchedEntity.name}" (${bestMatch.matchType}, ${(bestMatch.confidence * 100).toFixed(0)}%)`)
  }
...
export function matchProduct(
  ...
  if (bestMatch) {
    logger.debug(`🔍 Keyword match: PRODUCT "${message}" → "${bestMatch.matchedEntity.name}" (${bestMatch.matchType}, ${(bestMatch.confidence * 100).toFixed(0)}%)`)
  }
...
export function matchService(
  ...
  for (const service of services) {
    ...
    if (normalizedMessage === normalizedName || 
      ...
      logger.debug(`🔍 Keyword match: SERVICE "${message}" → "${service.name}"`)
      return {
 ...
 */
export function matchKnownEntities(
  message: string,
  entities: {
    categories: KnownEntity[]
    products: KnownEntity[]
