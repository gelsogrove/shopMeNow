...
export class LLMService {
  ...
  async handleMessage(
    ...
    // 8. 🌍 TRANSLATION LAYER - Final filter before sending to customer
    // 🚨 SKIP if called from delegation (Router will handle translation)
    if (skipTranslation) {
      logger.info("⏭️ Skipping Translation Layer (delegation mode)")
      ...
      try {
        ...
        debugInfo.translationBlocked = false
        debugInfo.translationReason = undefined
      } catch (error) {
        logger.error("❌ Translation Layer failed", error)
   ...
   * Ensures ALL messages go through the Translation Layer.
   *
  ...
  async handleNewUserWelcome(
    ...
    try {
      ...
      // 4. Translate through Translation Layer (MANDATORY)
  ...
  /**
   * Translate system message (welcome/WIP) through Translation Layer
  ...
  private async translateSystemMessage(
    ...
    try {
      logger.info(`🌐 Translating ${messageType} through Translation Layer`, {
        workspaceId,
        targetLanguage,
        messageType,
      ...
      const translationAgent = new TranslationAgent(prisma)
      const result = await translationAgent.process({
        workspaceId,
        message,
        targetLanguage,
        customerName,
  ...
  private async NewUser(
    ...
    // 3. Translate welcome message through Translation Layer
