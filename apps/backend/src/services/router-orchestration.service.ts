  ...
  private async generateRegistrationLink(
    ...
    try {
      ...
      try {
        ...
        logger.info(`📎 [RouterOrchestration] Created short registration link: ${finalLink}`)
        return finalLink
      } catch (shortError) {
        logger.warn("⚠️ [RouterOrchestration] Failed to create short URL, using long URL:", shortError)
