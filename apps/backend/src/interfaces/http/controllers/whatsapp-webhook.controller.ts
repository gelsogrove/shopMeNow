...
export class WhatsAppWebhookController {
  ...
  private async _receiveMessageLocked(req: Request, res: Response): Promise<void> {
    ...
    try {
      ...
      // 💰 BILLING / ACCESS guard — shared pipeline (channel-disabled, debug WIP,
      // PAUSED/credit-exhausted, trial, credit). Same for all providers.
      ...
      if (billingBlock) {
        res.status(billingBlock.statusCode).json(billingBlock.body ?? { status: billingBlock.status, code: billingBlock.code })
        return
      }
      ...
      res.status(result.statusCode).json(result.body ?? { status: result.status, code: result.code })
      return
    } catch (error: any) {
