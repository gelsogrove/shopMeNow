...
export class WorkspaceController {
  ...
  initializeWaapiInstance = async (
    ...
    try {
      ...
      logger.info('[WaAPI] Initializing instance:', { workspaceId, userId })

      const workspace = await this.workspaceService.initializeWaapiInstance(
        workspaceId,
        userId,
        phoneNumber,
        displayName
      ...
      logger.error('[WaAPI] Failed to initialize instance:', error)
  ...
  disconnectWaapiInstance = async (
    ...
    try {
      ...
      logger.info('[WaAPI] Disconnecting instance:', { workspaceId, userId })

      await this.workspaceService.disconnectWaapiInstance(workspaceId, userId)

      return res.status(200).json({ success: true, message: 'Instance disconnected' })
    } catch (error: any) {
      logger.error('[WaAPI] Failed to disconnect instance:', error)
      return res.status(400).json({ error: error.message })
    }
  }
  ...
  regenerateWaapiQr = async (
    ...
    try {
      ...
      logger.info('[WaAPI] Regenerating QR code:', { workspaceId, userId })

      const qrCodeData = await this.workspaceService.regenerateWaapiQr(workspaceId, userId)

      return res.status(200).json(qrCodeData)
    } catch (error: any) {
      logger.error('[WaAPI] Failed to regenerate QR code:', error)
      return res.status(400).json({ error: error.message })
    }
  }
}
