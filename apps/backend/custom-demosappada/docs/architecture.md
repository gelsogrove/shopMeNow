...
| `intake-question.ts` | Come la domanda dettata viene RESA: tradotta E privata delle parti che il profilo già risponde, in una sola chiamata guidata da una lista di fatti noti scritta dal codice; check di forma sul risultato, fallback al wording del tenant. |
...
| «se ti dico siamo senza macchina non puoi chiedermi 'Sarete senza macchina?' — è il minimo che un utente si aspetta» (2026-08-28) | `renderIntakeQuestion` (intake-question.ts) sostituisce la sola traduzione in entrambi i punti in cui la domanda va al composer: `knownFacts` (codice) elenca ciò che lo state contiene — conteggi, zeri inclusi, date, vincoli, interessi — e il modello riscrive la domanda dettata togliendo SOLO le parti già risposte; `renderingAcceptable` (forma: è ancora una domanda, non è cresciuta, non è vuota) altrimenti esce il wording del tenant. Nessuna chiamata extra quando lo state è vuoto. Il wording resta del tenant; la machine resta l'unica autorità su QUALE domanda. | `demosappada-intake-question.spec.ts` — bug live: «no nessuna» / «senza macchina» e la domanda `constraints` riletta per intero |
