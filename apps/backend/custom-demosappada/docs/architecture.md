...
| `provenance.ts` | `quoteAnchoredIn`: la citazione del modello (parole esatte dell'ospite) deve esistere nel messaggio. Provenienza dei numeri del gruppo (`partySaidAs`), stesso contratto di `dateSaidAs`. |
