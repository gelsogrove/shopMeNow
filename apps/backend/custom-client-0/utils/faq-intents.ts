...
// One responsibility per intent: detector + inlined prompt text + handler.
//
...
//
// NOTE: FAQ prompts are inlined here (no file I/O). To update a reply, edit the
// FAQ_PROMPTS map below. Each value is the exact customer-facing text.
...
// ── Inline FAQ prompts (no file I/O) ─────────────────────────────────────────
// Update these strings directly when customer-facing replies need to change.

const FAQ_PROMPTS: Record<FaqIntent, string> = {
  'discount-code':
    'Te ayudo con el código. ¿Me dices el código exacto tal como lo ves, incluyendo letras si las hay?',
  'invoice':
    'Para obtener tu factura, debes enviar un correo a olga@alberwaz.net con esta información: razón social, email, lavandería utilizada, CIF/NIF, dirección, fecha de uso, máquinas utilizadas y observaciones.',
  'buy-loyalty-card':
    'La tarjeta de fidelización se compra con 20 € en efectivo y solo funciona en la tienda donde se ha comprado. ¿En qué lavandería te encuentras?',
  'recharge-loyalty-card':
    'Para recargar la tarjeta, introdúcela en la central y sigue las instrucciones que aparecen en pantalla. Si aparece algún mensaje extraño, dímelo y lo revisamos juntos.',
  'hours-prices':
    'El horario general de atención al público es de 8:00 a 22:00 todos los días del año. En L\'Escala las máquinas se pueden utilizar de 7:00 a 23:00. Si me preguntas por el precio de una máquina concreta, tengo que revisarlo antes de confirmártelo.',
  'alarm-code':
    'Gracias por avisarnos. La máquina ha detectado una incidencia interna que necesitamos revisar manualmente. ⚠️ Voy a derivar tu caso a un operador. ¿Cómo te llamas?',
}

export function loadFaqPrompt(intent: FaqIntent): string {
  return FAQ_PROMPTS[intent]
}
