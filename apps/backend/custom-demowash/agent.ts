...
const TOOLS = [
  ...
  },
  {
    ...
    function: {
      ...
      description:
        'Read the CURRENT status of a specific machine from the remote laundromat system. Call this ONCE, as soon as you know location + machineType + machine number for a MACHINE problem (avería: puerta, alarma, no calienta, no arranca, puerta bloqueada…), BEFORE applying the troubleshooting procedure or escalating. Pass `status` = the display code the customer read on screen (or, if the customer cannot read it, the code that matches the symptom — pick from the codes documented in MACHINES). The tool returns the confirmed machine status; use THAT returned status as the single source of truth for the procedure you apply and for any escalation. Do NOT call this for payment-only issues (double charge, refund, invoice) or wash-advice questions — those are not machine reads.',
      parameters: {
        type: 'object',
        properties: {
          location: {
