...
const TOOLS = [
  ...
  },
  {
    ...
    function: {
      ...
      description:
        'Read the CURRENT status of a specific machine from the remote laundromat system. Call this ONCE, as the VERIFICATION step — AFTER the customer has tried the documented troubleshooting steps AND you have asked them what now appears on the display ("¿qué te aparece ahora en la pantalla?") and they have answered. Pass `status` = the display code the customer just read on screen after trying the steps. Do NOT call this the moment you have location + machineType + machine number: first let the customer try the documented procedure and tell you what they now see. EXCEPTION — direct-escalation cases with no step to try (secadora no calienta, ALERT/BLOCK, machine off): for those, call it as soon as you have location + machineType + machine number. Call it BEFORE asking the customer their name and before escalate_to_operator. The tool returns the confirmed machine status; use THAT returned status as the single source of truth in the escalation briefing. NOT for payment-only issues (double charge, refund, invoice) or wash-advice questions.',
      parameters: {
        type: 'object',
        properties: {
          location: {
