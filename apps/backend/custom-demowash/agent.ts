...
const TOOLS = [
  ...
  },
  {
    ...
    function: {
      ...
      description:
        'Read the CURRENT status of a specific machine from the remote laundromat system. Call this ONCE, AS SOON AS you have location + machineType + machine number for an incident heading to escalation — i.e. after the documented troubleshooting attempts have failed, or immediately for direct-escalation cases (secadora no calienta, ALERT/BLOCK, machine off). Call it the moment you have those three identifiers, BEFORE asking the customer their name and before escalate_to_operator. Pass `status` = the display code the customer read on screen (or, if the customer cannot read it, the code that matches the symptom — pick from the codes documented in MACHINES). The tool returns the confirmed machine status; use THAT returned status as the single source of truth in the escalation briefing. Do NOT call this before letting the customer try the documented steps, and NOT for payment-only issues (double charge, refund, invoice) or wash-advice questions.',
      parameters: {
        type: 'object',
        properties: {
          location: {
