# custom-demowash — Orchestration rules (read every turn)

Questo file è auto-caricato quando lavori sotto `apps/backend/custom-demowash/`. Leggilo PRIMA di ogni modifica. Le regole sotto sono non-negoziabili.
...
**Razionale**: l'LLM moderno (Claude 4.x) capisce il prompt strutturato. Le pezze regex nel codice sono il pattern che ha fatto esplodere demowash a 16k righe + F-log F1→F112. Non lo rifacciamo.
...
❌ Forbidden: XState, state machine, transition graphs, family detectors, root orchestrator. **Quello è demowash.**
✅ Allowed: 15 righe di `state.ts` con `getState`, `updateState`, `resetState`.

**Razionale**: non abbiamo transizioni discrete con regole rigide. Abbiamo solo "campi che si popolano in qualunque ordine". XState è la trappola che ha portato demowash alle 6 state machine + agent-bridge + family-detector. Non serve, non lo introduciamo.
...
❌ Forbidden: regole di tone in `agent.ts`, in handler dei tool, in questo CLAUDE.md. Codice che riformula la risposta del modello per "renderla più empatica" (rephrase layer di demowash — NO).
...
- **`[state]`** SessionState dopo ogni turno (es. `name=Marco location=Sants machine=5 display=OPEN`)
...
❌ Forbidden: mescolare config operativa e comportamentale (errore di demowash).
...
Tabella di riferimento per non re-introdurre pattern di demowash:

| Componente di demowash | Perché NON esiste qui |
|---|---|
...
## 🚨 Niente pezze (richiamo Andrea, 2026-05-23 da demowash)
...
- **Design espandibile**. Un fix per Demowash deve scalare ai prossimi clienti.
