...
import {
  ...
  guardNoChangeAwaitInvalidLadder,
...
import { guardResolutionAck } from './resolution-ack.js'
import { guardAnonymousEscalateClosure } from './anonymous-escalate-closure.js'
...
export const GUARD_PIPELINE: Guard[] = [
  // F112 — Resolution ack MUST run before everything else. When the router
  // classified turnMode='resolution', agent.ts has already fired markResolved
  // + resetMachineFacts via applyTurnModeGate. This guard emits the localised
  // "issue resolved, anything else?" ack and consumes pendingClosure so no
  // other guard re-interprets the (just-closed) state.
  guardResolutionAck,
  // F112 — Anonymous escalate closure: customer replies "gracias / grazie /
  // thanks / merci / obrigado / gràcies" while customerNameRequested=true.
  // Runs BEFORE any name-capture guard so the closure wins. Discount-code
  // and double-charge name flows have their own retry ladder and are
  // explicitly excluded inside the guard body.
  ...
  guardNoChangeNoCambio,
  guardNoChangeYesButBroken,
  // F112 — retry+escalate ladder for await-confirm when reply is NOT yes/no
  // AND NOT a display pivot. Must run AFTER the 3 above so legit yes/no/
  // display wins, BEFORE guardNoChangeAfterRetry (which handles the next
