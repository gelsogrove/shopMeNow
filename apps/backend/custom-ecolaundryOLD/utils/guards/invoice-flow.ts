  ...
  switch (flow) {
    ...
    case 'invoice-ask-email': {
      if (!EMAIL_RE.test(trimmed)) {
        // F111 — iron rule #10 corollary: gather step needs a 3-strikes ladder.
        // Previously this step looped forever ("Stay on the same step until a
        // valid email arrives"), so a customer typing garbage 50 times got 50
        // re-asks. Now: first invalid → re-ask, second invalid → escalate to
        // operator who collects the email manually.
        const step = nextRetryLadderStep(
          ar.state.invoiceEmailAskAttempts || 0,
          (n) => { ar.state.invoiceEmailAskAttempts = n },
        )
        if (step === 'escalate') {
          escalate(ar, 'Customer could not provide a valid invoice email after 2 attempts')
          requireCustomerName(ar)
          ar.state.pendingFlow = ''
          const escalateText = t('invoiceEmailRetryEscalate', lang(ar))
          const nameAsk = t('customerNameAsk', lang(ar))
          return {
            reply: `${escalateText} ${nameAsk}`,
            reason: 'invoice-email-retry-exhausted',
          }
        }
