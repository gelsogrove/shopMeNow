...
export const guardFaqClosure: Guard = (ar, userMessage) => {
  ...
  // F112-bis: 'd\'?\s*acord' previously failed on bare "daccord" (no apostrophe,
  // no space) because '\s*' requires whitespace. Replaced with '(?:[\s'']*?)'
  // so both "daccord", "d'accord", and "d accord" all match.
  const isAcknowledgment = /^(gracias|grazie|thanks|thank\s+you|merci|perfecto|perfect|perfetto|entendido|entendut|ent[èe]s|capito|got\s+it|ok|okay|vale|claro|de\s+acuerdo|d['']?accordo|d['']?accord|d['']?acord|adelante|s[ií]|yes|sim|oui|alright|va\s+bene|bien|tutto\s+bene)(?:\s+(?:gracias|gr[àa]cies|grazie|thanks|merci|obrigado))?$/i.test(lower)
