...
export function detectLanguageHeuristic(message: string): SessionState['language'] | null {
  ...
  // F111 — PT STRONG markers (parallel to CA STRONG / ES PUNCT). +20 lets
  // PT win against ES vocab matches on shared words (lavadora, secadora,
  // arranc, máquina, etc.). PT was structurally under-scored — the bot
  // replied in ES to PT messages like "Olá, paguei mas a máquina não
  // arrancou na Hortes" (Andrea random eval 2026-05-26).
  if (LANG_PT_STRONG_RE.test(normalized)) ptScore += 20
