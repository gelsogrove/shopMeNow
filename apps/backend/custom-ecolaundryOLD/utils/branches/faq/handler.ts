...
export const faqHandler: BranchHandler = async ({ message, ar, routerDetails, language }) => {
  ...
  if (!faqKey) {
    // F112-bis (Andrea random eval 2026-05-26 FR Scenario 2): before
    // falling back to unknownKey, check if the message is just a closure
    // token ("ok", "d'accord", "vale", "alright", "va bene"...). The
    // customer is acknowledging — not asking. Delegate to legacy so
    // guardFaqClosure can pick up the closure and emit the polite faqClosure
    // reply ("Perfect, anything else?"). Falling to unknownKey here would
    // emit "Je ne suis pas sûr d'avoir compris" which is wrong for a clear
    // closure.
    const lowerMsg = message.trim().toLowerCase().replace(/[.,!?¿¡]/g, '').trim()
    const CLOSURE_TOKEN_RE = /^(ok|okay|vale|d['']?accord|daccord|alright|bien|capito|entendido|entès|entes|capisco|got\s+it|de\s+acuerdo|va\s+bene|d['']?acord|ok\s+proviamo|gracias|grazie|thanks|merci|obrigado|obrigada|gr[àa]cies|gracies|sim|oui|s[íi])$/i
    if (CLOSURE_TOKEN_RE.test(lowerMsg)) {
      return { reply: '', handoff: 'delegate-to-legacy' }
    }
