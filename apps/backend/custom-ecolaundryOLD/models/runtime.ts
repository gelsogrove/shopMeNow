...
export type Settings = {
  ...
  /**
   * Statechart router (Phase 3 — see machines/PLAN.md). When true, the
   * root orchestrator in machines/root-orchestrator.ts takes the turn:
   * deterministic event detection → typed events → statechart transitions
   * → (i18nKey, vars, stage) → LLM polish layer → customer reply. Requires
   * useBranchRouter=true. Default false (opt-in).
   */
  useStatechartRouter?: boolean
  /**
  ...
  /**
   * When true, the rephrase LLM is also applied to display-flow turns
   * (DOOR, PUSH PROG, AL001, etc.) which are normally bypassed (F56).
   * Enables contextual recaps. Risk: may invent operational details.
   * Toggle in json/settings.json. Default: false.
   */
  rephraseDisplayFlow?: boolean
  /**
   * F75 (2026-05-22) — how many Phase B+ turns to show the problem summary
   * recap + reassurance once (at turns 3, 6, 9 …). Must be a positive integer.
   * Default: 3. Configurable per tenant via json/settings.json.
   */
  rephraseDisplayFlowRecapInterval?: number
  /**
