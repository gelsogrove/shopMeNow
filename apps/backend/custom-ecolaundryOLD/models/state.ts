...
export type SessionState = {
  ...
  // Counts invalid (non yes/no) replies while pendingFlow='no-change-await-confirm'.
  // The customer is expected to answer "¿la central te ha devuelto el cambio?"
  // with yes/no. If they instead correct an earlier fact ("sorry I meant the
  // washer", "number 5") the guards re-extract the fact via autoExtractFacts
  // but the yes/no question remains pending. After 2 invalid replies the
  // bot escalates to operator instead of looping forever (Scenario 2 EN
  // 2026-05-26: bot kept asking "Did the central give you the change back?"
  // on every turn the customer corrected an unrelated field).
  //   0 → first ask emitted (centralReturnedChange)
  //   1 → second turn without yes/no → re-ask with hint (centralReturnedChangeReask)
  //   ≥2 → escalate to operator
  // Reset to 0 on yes/no captured or by resetMachineFacts.
  noChangeAskAttempts: number
  ...
  // Counts how many times the invoice flow has asked for a valid email in
  // step `invoice-ask-email` without receiving one that matches EMAIL_RE.
  // Same 3-strikes ladder as displayAskAttempts (iron rule #10 corollary):
  //   0 → first ask "¿cuál es tu email?"
  //   1 → first invalid → re-ask "necesito un email válido…"
  //   ≥2 → escalate to operator (customer cannot supply a valid email; the
  //         operator collects it manually). Reset to 0 on valid email or
  //         when the invoice flow ends (escalation / restart).
  invoiceEmailAskAttempts: number
  // F113 — Counts how many turns in a row the customer's reply did NOT
  // produce a deterministic transition for the active flow-engine node
  // (`activeFlowId='non_parte'`, CONFIRMATION/CHOICE/ACTION not terminal).
  // Iron rule #10 catch-all for the LLM-tool flow engine: instead of letting
  // the LLM improvise a regression (re-asking known facts), `guardAdvanceMachineFlow`
  // re-emits the current node's prompt and applies the 3-strikes ladder:
  //   0 → first miss: re-emit prompt verbatim
  //   1 → second miss: prepend a soft hint (flowEngineReprompt i18n)
  //   ≥2 → escalate to operator (the customer is stuck in the flow)
  // Reset to 0 when the flow advances (forward progress) and by
  // resetMachineFacts / resetForNewIncident.
  flowReaskAttempts: number
  pendingFlow:
    | ''
