...
const cases: Case[] = [
  ...
  {
    name: 'F105 — [SUPERSEDED-BY-F112] heuristic still exists as fallback in resolveLanguageForTurn',
    run: () => {
      // SUPERSEDED-BY: F112 (2026-05-26) inverted the language source-of-truth.
      // Pre-F112: heuristic wins, router LLM only overrides when heuristic=null.
      // Post-F112: router LLM is AUTHORITATIVE at T1; heuristic is only used
      // as a fallback in resolveLanguageForTurn when the router has not yet
      // been consulted (boundary case: T1 with router disabled or failed).
      //
      // The original F105 bug (router misclassifies IT as ES on location names)
      // is now prevented by the extended router prompt (PT_STRONG markers +
      // 6-language exemplars) rather than by deferring to the heuristic.
      //
      // This pin now verifies the new contract: the heuristic still exists
      // as a fallback (so the bot is not language-blind when the router is
      // unavailable), AND the router decision overrides preferredLanguage
      // at T1 when present.
      const here = path.dirname(fileURLToPath(import.meta.url))
      const agentTs = fs.readFileSync(path.join(here, '..', '..', 'agent.ts'), 'utf8')
      // Heuristic must still be called as fallback in resolveLanguageForTurn.
      const fnRL = agentTs.indexOf('function resolveLanguageForTurn(')
      if (fnRL < 0) throw new Error('F105: resolveLanguageForTurn must exist in agent.ts')
      const fnBody = agentTs.slice(fnRL, fnRL + 2000)
      if (!fnBody.includes('detectLanguageHeuristic(')) {
        throw new Error('F105 (post-F112): resolveLanguageForTurn must still call detectLanguageHeuristic as fallback')
      }
      // Router decision must override preferredLanguage at T1 when present.
      if (!agentTs.includes('result.decision?.language')) {
        throw new Error('F105 (post-F112): T1 dispatch must consume result.decision.language')
      }
      if (!agentTs.includes('preferredLanguage = routerLang')) {
        throw new Error('F105 (post-F112): router language must be assigned to preferredLanguage at T1')
      }
    },
  },
  {
  ...
  // ── F111 — Invoice-ask-email retry ladder (rule #10 corollary) ───────────
  // Audit Andrea 2026-05-26: the `invoice-ask-email` gather step had no
  // 3-strikes counter. Customer with invalid email → infinite re-ask loop.
  // Pin requires the counter field exists, the guard uses nextRetryLadderStep,
  // and the escalate i18n key ships in all 6 catalogues (rule #8).
  {
    name: 'F111 — state.ts declares invoiceEmailAskAttempts counter field',
    run: () => {
      const src = fs.readFileSync(path.join(ECOLAUNDRY_ROOT, 'models/state.ts'), 'utf8')
      if (!/invoiceEmailAskAttempts:\s*number/.test(src)) {
        throw new Error('F111: models/state.ts must declare `invoiceEmailAskAttempts: number` field')
      }
    },
  },
  {
    name: 'F111 — createInitialState initialises invoiceEmailAskAttempts=0',
    run: () => {
      const src = fs.readFileSync(path.join(ECOLAUNDRY_ROOT, 'utils/state.ts'), 'utf8')
      if (!/invoiceEmailAskAttempts:\s*0/.test(src)) {
        throw new Error('F111: utils/state.ts:createInitialState must initialise invoiceEmailAskAttempts to 0')
      }
    },
  },
  {
    name: 'F111 — guardInvoiceFlow uses nextRetryLadderStep on invoice-ask-email step',
    run: () => {
      const src = fs.readFileSync(path.join(ECOLAUNDRY_ROOT, 'utils/guards/invoice-flow.ts'), 'utf8')
      // Helper must be imported from the canonical retry-ladder module.
      if (!/import\s*\{\s*nextRetryLadderStep\s*\}\s*from\s*['"]\.\/retry-ladder/.test(src)) {
        throw new Error('F111: invoice-flow.ts must import nextRetryLadderStep from ./retry-ladder.js')
      }
      // The email step must thread the counter through the ladder helper, not
      // a raw `return reAsk` loop. We scope the regex to the email case body.
      const emailCase = src.match(/case 'invoice-ask-email':\s*\{[\s\S]*?\n\s{0,6}\}/)?.[0] ?? ''
      if (!emailCase) {
        throw new Error('F111: cannot locate case `invoice-ask-email` in invoice-flow.ts')
      }
      if (!/nextRetryLadderStep\(\s*ar\.state\.invoiceEmailAskAttempts/.test(emailCase)) {
        throw new Error('F111: invoice-ask-email must call nextRetryLadderStep with state.invoiceEmailAskAttempts')
      }
      // On escalate path must call the named transitions, not inline mutation (rule #4).
      if (!/step === 'escalate'[\s\S]{0,500}escalate\(ar,/.test(emailCase)) {
        throw new Error('F111: invoice-ask-email escalate branch must call escalate(ar, reason)')
      }
      if (!/requireCustomerName\(ar\)/.test(emailCase)) {
        throw new Error('F111: invoice-ask-email escalate branch must call requireCustomerName(ar)')
      }
    },
  },
  {
    name: 'F111 — invoiceEmailRetryEscalate i18n key present in all 6 catalogues',
    run: () => {
      const langs = ['es', 'ca', 'en', 'it', 'fr', 'pt']
      for (const l of langs) {
        const p = path.join(ECOLAUNDRY_ROOT, 'json/i18n', `${l}.json`)
        const src = fs.readFileSync(p, 'utf8')
        if (!/"invoiceEmailRetryEscalate"\s*:/.test(src)) {
          throw new Error(`F111: i18n/${l}.json must define invoiceEmailRetryEscalate (rule #8)`)
        }
      }
    },
  },
  {
    name: 'F112 — statechart cascade gathers facts in canonical order (location→type→number→display)',
    run: () => {
      const src = fs.readFileSync(
        path.join(ECOLAUNDRY_ROOT, 'machines/trouble-machine.machine.ts'),
        'utf8',
      )
      // The evaluating.always[] must check facts in canonical order so a
      // multi-fact T1 (e.g. OPEN_INCIDENT + PROVIDE_TYPE) doesn't skip
      // askLocation. Pattern: location===null must be checked BEFORE
      // machineType===null in the always[] array.
      const cascade = src.match(/evaluating:\s*\{\s*[^}]*always:\s*\[([\s\S]*?)\]/)?.[1] ?? ''
      if (!cascade) {
        throw new Error('F112: cannot locate gathering.evaluating.always cascade')
      }
      const locIdx = cascade.indexOf('context.location === null')
      const typeIdx = cascade.indexOf('context.machineType === null')
      const numIdx = cascade.indexOf('context.machineNumber === null')
      if (locIdx < 0 || typeIdx < 0 || numIdx < 0) {
        throw new Error('F112: cascade must use explicit canonical-order guards (location/type/number ===null)')
      }
      if (!(locIdx < typeIdx && typeIdx < numIdx)) {
        throw new Error('F112: canonical order broken — must be location→type→number')
      }
    },
  },
  {
    name: 'F112 — LLM classifier prompt instructs multi-event extraction (composite messages)',
    run: () => {
      // Phase 4.4 (Andrea, 2026-05-27): the family-detector regex-based
      // composite branch was removed; the LLM classifier now extracts
      // ALL events in a single call. The pin verifies the prompt asks
      // for "ALL events" so composite messages keep working.
      const src = fs.readFileSync(
        path.join(ECOLAUNDRY_ROOT, 'machines/llm-event-classifier.ts'),
        'utf8',
      )
      if (!/Emit ALL events the customer expresses/.test(src)) {
        throw new Error('F112: classifier prompt must instruct multi-event extraction')
      }
      if (!/"events":\s*\[/.test(src)) {
        throw new Error('F112: classifier must produce a JSON events array')
      }
      // Verify the multi-event extraction function exists.
      if (!/export async function extractEventsWithLlm/.test(src)) {
        throw new Error('F112: classifier must export extractEventsWithLlm')
      }
    },
  },
  {
    name: 'F112 — projectFromStatechart wipes machine facts when active branch does not carry them',
    run: () => {
      const src = fs.readFileSync(
        path.join(ECOLAUNDRY_ROOT, 'utils/state-transitions.ts'),
        'utf8',
      )
      // The projection must explicitly clear displayState/machineType/
      // machineNumber when the active branch is faq/escalation/discount-
      // loyalty (these families don't carry machine facts in their
      // context, so leaving stale values from a previous trouble flow
      // creates briciole that break downstream guards reading AgentState).
      if (!/projectFromStatechart/.test(src)) {
        throw new Error('F112: projectFromStatechart must exist in state-transitions.ts')
      }
      if (!/displayState/.test(src)) {
        throw new Error('F112: projectFromStatechart signature must include displayState')
      }
      if (!/branchCarriesMachineFacts/.test(src)) {
        throw new Error('F112: projection must branch on whether the active branch carries machine facts')
      }
    },
  },
]
