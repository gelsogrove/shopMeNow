...
import { statechartTurn } from './machines/agent-bridge.js'
...
import { closeAsEscalated, consumeResolution, markResolved, releaseActiveFlow, releaseBranchOnFaqClosure, undoResolved } from './utils/state-transitions.js'
import { resetMachineFacts } from './utils/state.js'
...
export async function agentTurn(session: AgentSession, rawUserMessage: string): Promise<string> {
  ...
  // autoExtractFacts is the v1 legacy extractor — it greedily pulls
  // location/machineType/machineNumber/displayState from the raw text.
  // When the statechart pipeline is active, the statechart owns this
  // extraction (via the family-detector) and the legacy autoExtract
  // would clobber AgentState with false positives (e.g. "Av Diagonal
  // 123" → machineNumber=123 inside an invoice flow). Skip it when the
  // statechart router is on.
  if (!ar.runtime.settings?.useStatechartRouter) {
  ...
  // Phase 3 — Statechart pipeline (machines/, opt-in via settings.useStatechartRouter).
  // Runs BEFORE the legacy branch-router. The statechart handles turns it
  // recognises (a branch opener + at least one fact, OR a continuation of
  // an active statechart branch). When it returns null, we fall through
  // to the legacy pipeline so the bot keeps working for cases not yet
  // migrated. See machines/PLAN.md for the migration roadmap.
  if (ar.runtime.settings?.useStatechartRouter && ar.runtime.settings?.useBranchRouter) {
    try {
      const scReply = await statechartTurn(session, userMessage)
      if (scReply !== null) {
        return appendEscalationSummary(ar, scReply, history)
      }
    } catch (err) {
      // Defensive: if the statechart throws, fall back to legacy. We do
      // NOT swallow silently — log so we can investigate, but never let a
      // statechart bug break the customer's turn.
      logger.warn('Statechart pipeline failed, falling back to legacy', {
  ...
  if (ar.runtime.settings?.useBranchRouter) {
    ...
    if (decision) {
      // F112 — Deterministic gate: apply atomic side-effects (markResolved,
      // releaseBranch, etc.) BEFORE checking the branch handler's reply.
      // This is the SINGLE source of truth for cross-cutting conversation
      // control. The branch handler reply (if any) is OVERRIDDEN when the
      // gate produces an explicit reply (resolution-ack / unclear-ack).
      const earlyReply = applyTurnModeGate(ar, decision.turnMode)
      if (earlyReply) {
        return applyGuardOutcome(session, { reply: earlyReply, reason: `turn-mode-${decision.turnMode}` }, userMessage)
      }
  ...
  // F112 — L5 invariant: enforce session language as the FINAL step. If the
  // LLM agent loop or any earlier polish stage produced a reply in the wrong
  // language, re-render it via a dedicated translation LLM call. This is the
  // architectural backstop for iron rule #1 (no behavioural patches in
  // prompts/agent.txt — the guarantee lives in code).
  const languageEnforced = await enforceSessionLanguage(ar, polished)
  return appendEscalationSummary(ar, languageEnforced, history)
}
...
async function maybeDispatchBranch(
  ...
  // F112 — Router LLM is AUTHORITATIVE for language at T1 (architecture
  // refactor 2026-05-26). The previous design (F80 + F105) had the
  // heuristic win on confidence, but the heuristic was a hand-tuned regex
  // scoring system that drifted across 6 languages and required new
  // regex per edge case (PT not detected, IT mixed-message lost to ES,
  // EN with Spanish location name misclassified — see Andrea random eval
  // 2026-05-26). The LLM, which sees the FULL message + context + 6
  // languages natively, is now the source of truth.
  //
  // Heuristic remains as a FALLBACK in resolveLanguageForTurn() when the
  // router has not yet been consulted (boundary case: T1 with router
  // disabled or router failure). Once the router decision arrives, it
  // wins over the heuristic — that's the architectural inversion.
  //
  // T2+ stays STRICT-STICKY (F80 contract preserved): once preferredLanguage
  // is locked, no one overwrites it. The router result on T2+ is consumed
  // only for turnMode, not for language.
...
/**
 * F112 — Deterministic gate driven by the router LLM's `turnMode`.
 *
 * This is the single chokepoint that converts the router's semantic
 * understanding (resolution, topic-pivot, unclear) into deterministic state
 * mutations + early-exit replies, replacing the scattered regex detectors
 * (detectTroubleResolution / detectTroubleSwitchDuringFlow / detectFaqPause /
 * pivotIfTroubleSwitch / topicMachineTrouble) which suffered from O(N²)
 * pair-wise growth and 6-language regex maintenance.
 *
 * Returns:
 *   - A reply string when the turn is fully handled here (the dispatcher
 *     should short-circuit and emit this reply).
 *   - null when the turn should continue through the legacy pipeline
 *     (gather / new-incident / pure-closure / escalate / respond-and-continue).
 *
 * Side-effects (atomic, via named state transitions — iron rule #4):
 *   - resolution    → markResolved + resetMachineFacts (close the trouble)
 *   - topic-pivot   → releaseBranchOnFaqClosure (release sticky FAQ so legacy
 *                     pipeline resumes the trouble flow). NO early reply —
 *                     the trouble pipeline will produce the right answer.
 *   - unclear       → return the localised "non ho capito" reply.
 *   - others        → no-op (full pipeline runs as today).
 */
function applyTurnModeGate(
  ar: AgentRuntime,
  turnMode: import('./utils/router.js').TurnMode,
): string | null {
  const session = { ar }
  const lng = (ar.state.preferredLanguage as SupportedLanguage)
    ?? (ar.state.language as SupportedLanguage)
    ?? 'es'

  if (turnMode === 'resolution') {
    // Customer explicitly closed the trouble. Mark resolved + wipe machine
    // facts so the next turn does not re-arm guardAutoStartMachineFlow.
    // Then emit the localised "issue resolved, anything else?" ack directly
    // — F112 chokepoint: short-circuit the pipeline so no FAQ guard or LLM
    // loop re-interprets the (just-closed) state. Customer can open a new
    // topic on T+1 if needed (router will re-classify with fresh state).
    markResolved(ar)
    resetMachineFacts(ar.state)
    // Release branch so T+1 re-routes fresh (no sticky trouble-machine).
    if (ar.state.activeBranch) {
      ar.state.previousBranch = ar.state.activeBranch
      ar.state.activeBranch = null
    }
    // Consume pendingClosure immediately so T+1's autoExtract doesn't re-fire
    // post-resolution reset (which would wipe location/machine again).
    consumeResolution(ar)
    return t('al001Resolved', lng)
  }

  if (turnMode === 'topic-pivot') {
    // Customer explicitly returns to a prior trouble after a FAQ digression.
    // Release the sticky FAQ branch so dispatchTurnOne re-routes via the
    // router on T+1 (already done because we're inside the dispatcher),
    // AND let the legacy pipeline + trouble guards resume the flow.
    releaseBranchOnFaqClosure(ar)
    // Also release any stale active flow (F109 chokepoint complement).
    if (ar.state.activeFlowId) releaseActiveFlow(ar)
    return null
  }

  if (turnMode === 'unclear') {
    // Router could not understand the message. Bot says "ripeti per favore"
    // in the customer's language. NEVER ask for location/type/number — those
    // would compound the misunderstanding (Andrea: "se non capisce dica
    // ripeti per favore, non chieda dove è").
    //
    // Exception: when a gather flow is ACTIVE (pendingFlow set, or operator/
    // name capture in progress, or active flow engine step), the customer's
    // reply is the answer to a specific question the bot just asked. The
    // owning guard validates the answer and re-asks if invalid — overriding
    // to "non ho capito" here would short-circuit a legitimate retry.
    //
    // F112-bis (Andrea random eval 2026-05-26 Scenario 2 FR): also skip when
    // the customer just used the most common closure forms ("d'accord",
    // "ok", "vale", "ok proviamo", "alright", etc.). The router LLM
    // sometimes mis-classifies these as `unclear` because they're short
    // and ambiguous out of context. The deterministic guards (faqClosure,
    // anonymousEscalateClosure) own the closure semantics and will pick
    // them up downstream — never override with "non ho capito".
    const lowerMsg = (ar.state.lastUserMessage || '').trim().toLowerCase().replace(/[.,!?¿¡]/g, '').trim()
    const COMMON_CLOSURE_RE = /^(ok|okay|vale|d['']?accord|daccord|alright|bien|capito|entendido|entès|entes|capisco|got\s+it|de\s+acuerdo|va\s+bene|d['']?acord|ok\s+proviamo|ok\s+vamos|let['']?s\s+try|essayons|on\s+essaie|provem|provemos|tentemos)$/i
    if (COMMON_CLOSURE_RE.test(lowerMsg)) {
      return null
    }
    if (
      ...
      ar.state.activeStepId
    ) {
      return null
    }
    return t('unclearAck', lng)
  }
