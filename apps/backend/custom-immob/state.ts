...
export interface SessionState {
  // Operational
  name?:         string
  location?:     string            // branch/office the customer is dealing with (canonical city)
  operation?:    'buy' | 'rent'    // buy = comprar/comprare. rent = alquilar/affittare.
  ...
  // Appointment scheduling (property viewing OR franchising consultation)
  appointmentDate?:  string     // ISO format: YYYY-MM-DD
  appointmentTime?:  string     // HH:MM format
  appointmentType?:  string     // e.g. "viewing" | "franchising_consultation"

  // Profile (also mirrored to backend Customers via patches)
  companyName?: string
  address?:     string          // property address (for a valuation request) / customer address
  phone?:       string
  notes?:       string
    ...
    if (v !== undefined && v !== null && v !== '') {
      ...
      if (prev !== v) {
        ...
        if (mirror && MIRRORED_KEYS.includes(k)) {
          // companyName mirrors to the 'company' Customers column.
          const patchKey = (k === 'companyName' ? 'company' : k) as PatchKey
...
// collide across es/ca/it/pt/fr. We removed that detector entirely.
//
...
//     ambiguous message ("5", "Gràcia", "ok") never demotes the language.
//
...
export function formatStateForPrompt(state: SessionState): string {
  ...
  if (state.location) fields.push(`Active office: ${state.location}`)
  if (state.operation) fields.push(`Operation: ${state.operation}`)
  ...
  const languageBlock = [
    ...
    hasLang
      ...
      // flip it. This is what stops "Gràcia" / "EIX-101" / "3" from changing
      ...
      : `- No language is set yet (this is the first message). Detect the language from the customer's message — even a single word like "Ciao", "merci", "gracias" is enough. If the message carries NO language signal at all (a bare number "3", a name "Andrea", a place name "Barcelona", "ok", a listing reference like "EIX-101"), use ${seed}.`,
    hasLang
      ? `- ONLY switch away from ${state.language} if the customer's latest message is a REAL sentence (roughly 3+ meaningful words) clearly written in another language. A single word, a name, a place ("Gràcia"/"Eixample"), a number, an emoji, "ok", or a listing reference ("EIX-101") is NOT enough to switch — keep ${state.language}.`
      : `- Spanish ("es") is the business default when the very first message is genuinely undecidable.`,
    '- A listing REFERENCE or a price/number the customer quotes ("EIX-101", "300.000 €", "3 habitaciones") is DATA, not language — it never changes the conversation language.',
    '- ⚠️ The LOCATIONS / data blocks contain Catalan place names and labels used AS DATA. Judge the language ONLY from what the CUSTOMER wrote — never let Catalan data pull your reply into Catalan. Copy data values (prices, sizes, references, zones) but write all labels/sentences in the customer\'s language.',
    '',
