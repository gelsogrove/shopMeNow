...
//      (email, CIF, NIF, IBAN, card-16-digits, card-last-4-with-context,
...
// Italian codice fiscale: 6 letters + 2 digits + 1 month letter + 2 digits +
// 1 letter + 3 digits + 1 control letter (16 alphanumerics, fixed shape).
const CODICE_FISCALE_RE = /\b[A-Z]{6}\d{2}[A-EHLMPRST]\d{2}[A-Z]\d{3}[A-Z]\b/gi
...
// detect the venue deterministically here, by matching one of these 3 fixed
...
const CANONICAL_VENUES = ['Navigli', 'Isola', 'Monza'] as const

// Match a venue accent-insensitively and as a whole word. Returns the canonical
// spelling or null.
...
export function preScanAndRedact(input: string): RedactionResult {
  ...
  // IBAN (before card patterns to avoid digit-run overlaps)
  ...
  // Codice fiscale
  s = s.replace(CODICE_FISCALE_RE, (m) => {
    if (!captured.codiceFiscale) captured.codiceFiscale = m
...
export function deRedactWithState(input: string, state: SessionState): string {
  ...
  const subs: Array<[string | undefined, string]> = [
    ...
    [state.codiceFiscale, '[CODICE_FISCALE]'],
    [state.phone, '[PHONE]'],
...
export function processIncomingMessage(
  ...
  if (captured.codiceFiscale) patch.codiceFiscale = captured.codiceFiscale
...
export function substitutePlaceholders(text: string, state: SessionState): string {
  ...
  const subs: Array<[string, string | undefined]> = [
    ...
    ['[EMAIL]', state.email],
    ['[CODICE_FISCALE]', state.codiceFiscale],
    ['[IBAN]', state.iban],
