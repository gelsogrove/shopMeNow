...
// Cart line item — a service (treatment, with a duration) or a product (no
// duration). Held in memory only and cleared on booking (see resetCart).
export interface CartItem {
  kind:      'service' | 'product'
  name:      string            // e.g. "Pulizia viso profonda", "Siero Acido Ialuronico"
  price:     number            // euros
  durationMin?: number         // services only — used to compute the appointment end time
  qty?:      number            // products only — default 1
}

export interface SessionState {
  // Operational
  name?:        string
  location?:    string          // Canonical sede: 'Navigli', 'Isola', 'Monza'
  service?:     string          // Treatment the customer is interested in (free text)
  // In-memory cart: services + products the customer is accumulating this
  // session. NEVER persisted to a DB — cleared on a successful booking.
  cart?:        CartItem[]
  ...
  // Appointment scheduling (treatment booking)
  appointmentDate?:  string     // ISO format: YYYY-MM-DD
  appointmentTime?:  string     // HH:MM format
  appointmentType?:  string     // e.g. "treatment_booking"
...
// ── Cart helpers ─────────────────────────────────────────────────────────────
// The cart is an in-memory array on SessionState. `setCart` REPLACES it (the
// LLM passes the full desired cart each time — merge semantics don't fit a list
// the customer can also remove items from). `resetCart` clears it after a
// successful booking, so the next conversation starts empty.

export function setCart(sessionId: string, items: CartItem[]): SessionState {
  const e = entry(sessionId)
  e.state.cart = items
  return e.state
}

export function resetCart(sessionId: string): void {
  const e = entry(sessionId)
  e.state.cart = undefined
}
...
// generate every turn.
//
// Policy: Italian ('it') is the business default, used only as the seed when no
...
export function formatStateForPrompt(state: SessionState): string {
  ...
  if (state.location) fields.push(`Active sede: ${state.location}`)
  if (state.service) fields.push(`Service of interest: ${state.service}`)
  if (state.cart && state.cart.length > 0) {
    const lines = state.cart.map((i) => {
      const qty = i.kind === 'product' && (i.qty ?? 1) > 1 ? ` x${i.qty}` : ''
      const dur = i.durationMin ? ` (${i.durationMin}min)` : ''
      return `${i.name}${qty} — ${i.price}€${dur}`
    })
    fields.push(`Cart (${cartTotal(state.cart)}€ total): ${lines.join('; ')}`)
  }
  if (state.appointmentDate) {
    fields.push(`Booked appointment: ${state.appointmentDate} ${state.appointmentTime ?? ''}`.trim())
  }
  if (state.language) {
  ...
  const languageBlock = [
    ...
    hasLang
      ? `- ONLY switch away from ${state.language} if the customer's latest message is a REAL sentence (roughly 3+ meaningful words) clearly written in another language. A single word, a name, a sede ("Navigli"/"Isola"/"Monza"), a number, an emoji or "ok" is NOT enough to switch — keep ${state.language}.`
      : `- Italian ("it") is the business default when the very first message is genuinely undecidable.`,
    '- ⚠️ The LOCATIONS / data blocks are written in Italian and contain Italian sede names, service names and labels used AS DATA. Judge the language ONLY from what the CUSTOMER wrote — never let Italian data pull your reply into Italian. Copy data values (prices, hours, service names) but write all labels/sentences in the customer\'s language.',
    '',
...
export function formatStateOneLine(state: SessionState): string {
  ...
  const PII_KEYS = new Set<keyof SessionState>(['email', 'codiceFiscale', 'iban', 'cardFull', 'cardLast4', 'phone'])
  const parts: string[] = []
