/**
 * Duplicate the demowash demo Workspace as "DemoRealEstate" (real-estate) so
 * /demo/demorealestate resolves, owned by a given user.
 *
 ...
 * `customChatbotId` equals the URL slug. So for /demo/demorealestate to work, ONE
 * workspace must exist with customChatbotId="demorealestate". The custom chatbot module
 * (apps/backend/custom-demorealestate/) is standalone and prompt-driven — it needs no
 ...
 * demo-chat endpoint, not a real WhatsApp channel. Sets name="DemoRealEstate",
 * slug="demorealestate", customChatbotId="demorealestate", owner = the --owner user, and
 ...
 * write. Idempotent: if a workspace with customChatbotId="demorealestate" already
 * exists, it reports it and exits without creating a duplicate.
 *
 * Run (LOCAL, from project root):
 *   dotenv -e .env -- tsx apps/backend/scripts/create-demorealestate-demo-workspace.ts
 *   dotenv -e .env -- tsx apps/backend/scripts/create-demorealestate-demo-workspace.ts --apply
 *
 ...
 *     tsx apps/backend/scripts/create-demorealestate-demo-workspace.ts --owner gelsogrove@gmail.com
...
// Defaults create DemoRealEstate; override via flags to create any custom-chatbot demo
// (e.g. --chatbot demowash --name DemoWash --slug demowash --business laundry).
const CHATBOT_ID = flag("chatbot") || "demorealestate"
const NAME = flag("name") || "DemoRealEstate"
...
async function main() {
  // Idempotency: never create a second DemoRealEstate workspace.
  ...
  // DemoRealEstate is still created standalone.
