# Ecolaundry — Architettura

Documento di riferimento per il chatbot Ecolaundry. Tutto quello che serve sapere per estendere, mantenere e portare in produzione il bot.
...
Custom-ecolaundry è nato dal paradigma opposto (LLM = inaffidabile, serve un guard layer deterministico per ogni comportamento). Il risultato è ~5000 righe di codice, 6 state machine XState, 30+ guard, ~1600 unit test, F-log F1→F112.

Ecolaundry parte dall'osservazione empirica che Claude 4.x (Haiku 4.5, Sonnet 4.6, Opus 4.7) con un system prompt ben strutturato gestisce nativamente:
...
- Lista codici display (`SELECT`, `CHOICE PROG`, `ERROR DOOR`, `ALARM`, `ALARM DOOR`, `ERR-001`)
...
**Pagamento**: tarjeta, contanti, App Ecolaundry
```
...
Questo blob (~28k token per Ecolaundry a 6 sedi) è **il system prompt cached**. Non cambia mai tra turni della stessa sessione, né tra sessioni diverse, finché i file su disco non vengono editati.
...
Per i numeri di Ecolaundry (6 sedi, ~28k token):
...
Limite pratico: ~150 location o ~30 modelli macchina (totale prompt ~150k token, comincia a essere lento e costoso anche cached). Oltre, si introduce RAG: vector search → recupera top-K blocchi rilevanti → inietta nel prompt al momento. Ma per Ecolaundry siamo lontanissimi dal limite.
...
interface SessionState {
  ...
  machine?:     number                                   // 5
  displayCode?: string                                   // "DOOR" | "SELECT" | "ALARM" | ...
...
- Sanitizzazione del messaggio utente: strip caratteri di controllo, zero-width, bidi (come `sanitizeUserMessage` di ecolaundry)
...
Stampa lo `SessionState` dopo ogni reply per debug (es. `[state] name=Marco location=Sants machine=5 display=DOOR`).
...
Stesso pattern di custom-ecolaundry. Ogni entry:
...
**Nessun file di settings JSON come ecolaundry**: tutta la configurazione di comportamento sta nel prompt, tutta la configurazione operativa nelle env var.

---

## 12. Cosa NON c'è (e perché)

| Componente di ecolaundry | Perché non esiste in ecolaundry |
|---|---|
...
- ecolaundry: ~5000 righe TypeScript + ~2000 righe JSON config
- ecolaundry: ~250 righe `agent.ts` + ~28k token markdown (i prompt sono "documenti", non codice)
...
8. **Observability**: log `llm.call` con caller/latency/cost (come `llm-fetch.ts` di ecolaundry).
...
Casi in cui ecolaundry deve evolvere:
...
3. **Compliance auditabilità forte** (es. finanza, sanità): serve poter ricostruire deterministicamente "perché il bot ha detto X". Lì un layer di rule engine + LLM-as-second-opinion ha senso. Per Ecolaundry non è il caso.
...
- `../custom-ecolaundry/` — implementazione legacy (riferimento storico)
- `../custom-ecolaundry/json/dryer_ed340.json` — esempio di flow engine deprecato (mostra cosa NON fare)
...
### Costo per turno (Ecolaundry con Haiku 4.5)
...
| Voce annua | ecolaundry | ecolaundry | Delta |
|---|---|---|---|
...
- ecolaundry (1 LLM call): ~800-1500ms p50, ~2500ms p95
- ecolaundry (4 LLM call sequenziali): ~3000-4500ms p50, ~7000ms p95
...
Il rischio principale di ecolaundry è che il modello inventi dati (prezzi, orari, codici) invece di pescarli dal prompt. Strategie deterministiche per ridurlo, in ordine di efficacia:
...
Configurazione confermata per ecolaundry:
...
**Per Ecolaundry**: non lo facciamo. Prezzi pubblici, errore tollerabile, ROI negativo.
...
## 19. Confronto sintetico con custom-ecolaundry

### Per cosa scegliere ecolaundry
...
### Per cosa scegliere ecolaundry (paradigma legacy)
...
### Per la maggioranza dei casi: ecolaundry + 4 layer di sicurezza

La via realistica per produzione è ecolaundry + i layer della §13 prioritari:
1. PII redaction
...
Totale ~500 righe extra. Si resta ~30× più snelli di ecolaundry, con garanzie operative comparabili sui rischi reali.

### Cosa NON copiare da ecolaundry
...
### Cosa COPIARE da ecolaundry (principi, non implementazioni)
