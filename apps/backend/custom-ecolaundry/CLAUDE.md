...
| B8 | **F87 follow-up — validation al boot per `metadata.payment`** (Andrea 2026-05-23). Verificare che per ogni location esistano `methods` non-vuoto + subset di `{coins,bills,fidelity,card}` + `tpvExact` null o int positivo. Oggi se qualcuno edita `locations.json` malformato (es. `methods: "card"` invece di array, o `tpvExact: "7€"` invece di int), il bot fallisce silenziosamente in `formatPaymentSignals` (no append, no error). | Se in produzione succede che qualcuno editi `locations.json` malformato; OR audit pre-go-live dei dati metadata. | Estendere `utils/runtime.ts:validateSettings` (o creare `validateLocations` chiamato da `loadRuntime`). Fail-fast al boot con log esplicito: per ogni location loop su `metadata.payment`, valida shape, throw error con `<key>.metadata.payment.<field>: expected X, got Y`. |
| B7 | **F87 follow-up — risolvere `weightKg: null` per Goya secadoras S1/S2/S3** (Andrea 2026-05-23, ex-AD3 deferred). Il CSV `preus.csv` non specifica il peso fisico delle 3 secadoras Goya alla fonte. Cliente vede `- **Secadoras**: 2€/15min` senza menzione del peso, mentre per altre location vede `- **Secadoras** 20kg: 2€/15min` → inconsistenza UX. | Telefonata operatore Olga / proprietà Goya per ottenere il dato fisico reale. | Solo data fix: aggiornare `json/locations.json:Goya.metadata.machines.dryers[].weightKg` (3 entries S1/S2/S3) con il valore corretto. No code change. Aggiornare anche `docs/csv/preus.csv` riga GOYA Peso secadoras per allineare CSV-as-source-of-truth. |
| B6 | **F87 follow-up — extras (aclarado/lavado 1€) + extended tier dryer L'Escala (5€/25min)** (Andrea 2026-05-23, ex-AD2 deferred). `metadata.extras` già esiste in `locations.json` per L'Escala/PlatjaDAro ma non viene mostrato nel reply prezzi. L'Escala ha anche `dryers[].extended: "5€/25min"` che non viene mai esposto. | Primo cliente in produzione che chiede "c'è extra cost?" / "5€ programma?" / "c'è opzione lunga?" (real-bug evidence required). | Estendere `formatWasherPrices`/`formatDryerPrices` con append condizionale: se `metadata.extras` definito → `\n💧 Extra aclarado: {X}€ \| Extra lavado: {Y}€`. Se `dryers[].extended` presente → mostrare come 3rd tier nella lista. 2 nuove i18n keys (`paymentExtras`, `dryerExtendedTier`) × 6 lingue. Sibling test esteso. F-log entry come F88. |
