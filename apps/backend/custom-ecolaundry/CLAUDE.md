...
| D2 | **Extract LLM system prompts from TS consts to `prompts/*.txt` files** for runtime editability (same pattern as `prompts/agent.txt`). Today: `agent.txt` is the only prompt loaded from disk (re-read at every `loadPromptBundle`); the others are hardcoded TS const strings → require restart to change. Touch points: (1) `utils/router-prompt.ts:ROUTER_SYSTEM_PROMPT` → `prompts/router.txt`; (2) `utils/agent-rephrase.ts:REPHRASE_SYSTEM_PROMPT` → `prompts/rephrase.txt`; (3) `utils/operator-briefing.ts:BRIEFING_SYSTEM_PROMPT` → `prompts/operator-briefing.txt`; (4) inline string in `utils/flow-engine.ts:classifyChoiceViaLLM` → `prompts/flow-classify.txt`. Extend `loadPromptBundle` in `utils/agent-prompt.ts` to load the new files; keep TS const as fallback when the file is missing. Operator can iterate on tone without redeploy (same UX as `prompts/agent.txt` today). | Andrea wants to tune any of these prompts without redeploy. | Start in `utils/agent-prompt.ts:loadPromptBundle` — replicate the `agent.txt` loading pattern for the four new files. |
