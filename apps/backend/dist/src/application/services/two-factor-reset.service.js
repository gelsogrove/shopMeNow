...
exports.twoFactorResetService = exports.TwoFactorResetService = void 0;
...
const adminResetCounts = new Map();
...
class TwoFactorResetService {
    ...
    getTransporter() {
        if (!this.transporter) {
            ...
            logger_1.default.info(`📧 Setting up email transporter: ${config.host}:${config.port} (secure: ${config.secure})`);
     ...
     * - Sends email
     */
    createResetToken(userId, adminId, adminEmail) {
        return __awaiter(this, void 0, void 0, function* () {
            // SEC-10: Prevent admin from resetting own 2FA
            if (userId === adminId) {
                throw new error_middleware_1.AppError(400, 'Cannot reset your own 2FA. Use password reset if locked out.');
            }
            ...
            if (adminRate) {
                ...
                if (adminRate.resetAt > hourAgo && adminRate.count >= MAX_ADMIN_RESETS_PER_HOUR) {
                    throw new error_middleware_1.AppError(429, 'Too many reset requests. Maximum 10 resets per hour.');
                }
            ...
            if (!user.twoFactorEnabled) {
                throw new error_middleware_1.AppError(400, 'User does not have 2FA enabled');
            }
            ...
            // Create token record
            const resetToken = yield this.prisma.twoFactorResetToken.create({
                data: {
                    userId,
                    token,
                    expiresAt,
            ...
            // SEC-12: Audit log
            logger_1.default.info(`2FA reset initiated by admin ${adminEmail} for user ${user.email} (token: ${token.substring(0, 8)}...)`);
            // Send email with reset link
            yield this.sendResetEmail(user.email, token, adminEmail, user.firstName);
            return {
     ...
     */
    createEnableToken(userId, adminId, adminEmail) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (adminRate) {
                const hourAgo = new Date(now.getTime() - 3600000);
                if (adminRate.resetAt > hourAgo && adminRate.count >= MAX_ADMIN_RESETS_PER_HOUR) {
                    throw new error_middleware_1.AppError(429, 'Too many requests. Maximum 10 per hour.');
                }
                if (adminRate.resetAt <= hourAgo) {
                    adminResetCounts.set(adminId, { count: 1, resetAt: now });
                }
            ...
            if (user.twoFactorEnabled) {
                throw new error_middleware_1.AppError(400, 'User already has 2FA enabled. Use Reset 2FA instead.');
            }
            // Generate token
            const token = crypto.randomUUID();
            const expiresAt = new Date(Date.now() + 3600000); // 1 hour
            // Create token record
            const enableToken = yield this.prisma.twoFactorResetToken.create({
                data: {
                    userId,
                    token,
                    expiresAt,
            ...
            // Audit log
            logger_1.default.info(`2FA enable initiated by admin ${adminEmail} for user ${user.email} (token: ${token.substring(0, 8)}...)`);
            // Send email with setup link
            yield this.sendEnableEmail(user.email, token, adminEmail, user.firstName);
            return {
    ...
    validateToken(token, ipAddress) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const resetToken = yield this.prisma.twoFactorResetToken.findFirst({
                where: { token },
                include: { user: true },
            });
            if (!resetToken) {
     ...
     */
    verifyPassword(token, email, password) {
        return __awaiter(this, void 0, void 0, function* () {
            const resetToken = yield this.prisma.twoFactorResetToken.findFirst({
                where: { token },
                include: { user: true },
            ...
            if (resetToken.lockedUntil && new Date() < resetToken.lockedUntil) {
                const remainingMinutes = Math.ceil((resetToken.lockedUntil.getTime() - Date.now()) / 60000);
                return {
                    success: false,
            ...
            if (resetToken.user.email.toLowerCase() !== email.toLowerCase()) {
                yield this.incrementPasswordAttempts(resetToken);
                return { success: false, error: 'Invalid credentials' };
            }
            ...
            // Verify password
            const passwordValid = yield bcrypt_1.default.compare(password, resetToken.user.passwordHash);
            if (!passwordValid) {
                yield this.incrementPasswordAttempts(resetToken);
                return { success: false, error: 'Invalid credentials' };
            }
            // Reset failed attempts on success
            yield this.prisma.twoFactorResetToken.update({
            ...
            logger_1.default.info(`Password verified for 2FA reset, user: ${email} (token: ${token.substring(0, 8)}...)`);
            return {
                success: true,
                tempToken,
            };
     ...
     */
    complete2FASetup(tempToken, newSecret, totpCode) {
        return __awaiter(this, void 0, void 0, function* () {
            // Verify temp token
            let decoded;
            try {
            ...
            // Get the reset token
            const resetToken = yield this.prisma.twoFactorResetToken.findUnique({
            ...
            const speakeasy = yield Promise.resolve().then(() => __importStar(require('speakeasy')));
            ...
            logger_1.default.info(`2FA reset completed for user ${resetToken.userId} (token: ${resetToken.token.substring(0, 8)}...)`);
            return { success: true };
        });
    }
    /**
     ...
     */
    incrementPasswordAttempts(resetToken) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (shouldLock) {
                logger_1.default.warn(`2FA reset token locked due to failed password attempts (token: ${resetToken.token.substring(0, 8)}...)`);
            }
        });
    }
    /**
     ...
     */
    sendResetEmail(userEmail, token, adminEmail, firstName) {
        return __awaiter(this, void 0, void 0, function* () {
...
`;
            try {
                ...
                logger_1.default.info(`2FA reset email sent to ${userEmail}`);
            }
            catch (error) {
                logger_1.default.error(`Failed to send 2FA reset email to ${userEmail}:`, error);
                throw new error_middleware_1.AppError(500, 'Failed to send reset email');
            }
        });
    }
    /**
     ...
     */
    sendEnableEmail(userEmail, token, adminEmail, firstName) {
        return __awaiter(this, void 0, void 0, function* () {
            const frontendUrl = process.env.FRONTEND_URL || 'http://localhost:3000';
...
`;
            try {
                ...
                logger_1.default.info(`2FA enable email sent to ${userEmail}`);
            }
            catch (error) {
                logger_1.default.error(`Failed to send 2FA enable email to ${userEmail}:`, error);
                throw new error_middleware_1.AppError(500, 'Failed to send setup email');
            }
        });
    }
    /**
    ...
    cleanupExpiredTokens() {
        return __awaiter(this, void 0, void 0, function* () {
            const result = yield this.prisma.twoFactorResetToken.deleteMany({
                where: {
                    expiresAt: { lt: new Date() },
                },
            });
            if (result.count > 0) {
                logger_1.default.info(`Cleaned up ${result.count} expired 2FA reset tokens`);
            }
            return result.count;
        });
    }
}
exports.TwoFactorResetService = TwoFactorResetService;
const twoFactorResetService = (prisma) => new TwoFactorResetService(prisma);
exports.twoFactorResetService = twoFactorResetService;
//# sourceMappingURL=two-factor-reset.service.js.map