...
class TemplateEngineService {
    constructor() {
        logger_1.default.info("✅ TemplateEngineService initialized");
    }
    /**
     ...
     */
    processUnlessBlocksIteratively(template, variables) {
        let result = template;
     ...
     */
    processInnermostUnless(template, variables) {
     ...
     */
    processIfBlocksIteratively(template, variables) {
        let result = template;
        ...
        if (iterations >= maxIterations) {
            logger_1.default.warn("⚠️ TemplateEngine: Max iterations reached, possible malformed template");
        }
        return result;
    }
    /**
     ...
     */
    processInnermostIfBlock(template, variables) {
        ...
        const ifElseRegex = /\{\{#if\s+(\w+)\}\}((?:(?!\{\{#if\s)(?!\{\{\/if\}\})[\s\S])*?)\{\{else\}\}((?:(?!\{\{#if\s)(?!\{\{\/if\}\})[\s\S])*?)\{\{\/if\}\}/;
        ...
        if (ifElseMatch) {
            ...
            logger_1.default.debug(`Processing if-else block: condition=${condition}, value=${value}, isTruthy=${isTruthy}`);
        ...
        const simpleIfRegex = /\{\{#if\s+(\w+)\}\}((?:(?!\{\{#if\s)(?!\{\{else\}\})(?!\{\{\/if\}\})[\s\S])*?)\{\{\/if\}\}/;
        const simpleIfMatch = template.match(simpleIfRegex);
        if (simpleIfMatch) {
            const [fullMatch, condition, content] = simpleIfMatch;
            const value = variables[condition];
            const isTruthy = this.isTruthy(value);
            logger_1.default.debug(`Processing simple if block: condition=${condition}, value=${value}, isTruthy=${isTruthy}`);
        ...
        if (template.includes("{{#if")) {
            logger_1.default.warn("⚠️ TemplateEngine: {{#if}} found but no valid block matched - possible malformed template");
        }
        return template;
    }
    /**
