...
exports.LLMFormatterService = void 0;
exports.getLLMFormatter = getLLMFormatter;
...
function buildSystemPrompt(options) {
    ...
    const chatbotName = (options === null || options === void 0 ? void 0 : options.chatbotName) || (options === null || options === void 0 ? void 0 : options.botName) || "Assistente";
    const businessType = (options === null || options === void 0 ? void 0 : options.businessType) || "other";
...
- Adatta il linguaggio e gli esempi al contesto del settore quando appropriato`;
    ...
    if ((options === null || options === void 0 ? void 0 : options.botIdentity) && options.botIdentity.trim() !== "") {
        prompt += `
...
Ricorda: Mantieni la personalità nel TONO, ma non modificare i DATI. Sii sempre caloroso e umano!`;
    ...
    if ((options === null || options === void 0 ? void 0 : options.isFirstMessage) && (options === null || options === void 0 ? void 0 : options.customerName)) {
        prompt += `
...
- Sii naturale, mai robotico!`;
    ...
    if ((options === null || options === void 0 ? void 0 : options.customAiRules) && options.customAiRules.trim() !== "") {
        prompt += `
...
const formatDisplayPrice = (value, fallback = "€0.00") => {
    if (typeof value !== "number" || !isFinite(value)) {
        return fallback;
    }
    return (0, pricing_1.formatRoundedCurrency)(value, {
...
class LLMFormatterService {
    constructor(prisma) {
        ...
        this.model = "openai/gpt-4o-mini"; // Fast, cheap, good for formatting
        this.openRouterApiKey = process.env.OPENROUTER_API_KEY || "";
        if (!this.openRouterApiKey) {
            logger_1.default.warn("⚠️ OPENROUTER_API_KEY not set - LLM formatting will fail");
        }
    }
    /**
     ...
     */
    format(response_1) {
        return __awaiter(this, arguments, void 0, function* (response, targetLanguage = "it", conversationHistory, options) {
            var _a, _b, _c, _d, _e, _f;
            const startTime = Date.now();
            logger_1.default.info("📝 [LLMFormatter] Formatting response", {
                type: response.type,
                targetLanguage,
            ...
            if (cached) {
                logger_1.default.info("📝 [LLMFormatter] Used cached template", {
                    type: response.type,
            ...
            logger_1.default.info("📝 [LLMFormatter] Options received", {
                hasBotIdentity: !!(options === null || options === void 0 ? void 0 : options.botIdentity),
                botIdentityLength: ((_a = options === null || options === void 0 ? void 0 : options.botIdentity) === null || _a === void 0 ? void 0 : _a.length) || 0,
                hasCustomAiRules: !!(options === null || options === void 0 ? void 0 : options.customAiRules),
                customerName: options === null || options === void 0 ? void 0 : options.customerName,
                isFirstMessage: options === null || options === void 0 ? void 0 : options.isFirstMessage,
                botName: options === null || options === void 0 ? void 0 : options.botName,
            });
            if (options === null || options === void 0 ? void 0 : options.customAiRules) {
                logger_1.default.info("📝 [LLMFormatter] Using custom AI rules", {
                    rulesLength: options.customAiRules.length,
                });
            }
            if (options === null || options === void 0 ? void 0 : options.botIdentity) {
                logger_1.default.info("📝 [LLMFormatter] Using bot personality", {
            ...
            if (conversationHistory && conversationHistory.length > 0) {
                ...
                const relevantHistory = conversationHistory.filter(msg => msg.role === "system" || // System context (group mappings, cart state)
                ...
                logger_1.default.info("📝 [LLMFormatter] Including conversation history", {
            ...
            try {
                const llmResponse = yield axios_1.default.post(`${this.openRouterBaseUrl}/chat/completions`, {
                    model: this.model,
                    messages,
                ...
                let text = ((_c = (_b = llmResponse.data.choices[0]) === null || _b === void 0 ? void 0 : _b.message) === null || _c === void 0 ? void 0 : _c.content) || "";
                const tokensUsed = ((_d = llmResponse.data.usage) === null || _d === void 0 ? void 0 : _d.total_tokens) || 0;
                ...
                if (jsonMatch && jsonMatch[1]) {
                    try {
                        ...
                        logger_1.default.info("📝 [LLMFormatter] Using LLM-generated groupMapping (semantically meaningful)", {
                        ...
                        logger_1.default.warn("⚠️ [LLMFormatter] Failed to parse LLM group mapping JSON, falling back to code", {
                ...
                if (!groupMapping && ((_e = response.data) === null || _e === void 0 ? void 0 : _e.groupMapping)) {
                    groupMapping = response.data.groupMapping;
                    logger_1.default.info("📝 [LLMFormatter] Using CODE-computed groupMapping (fallback)", {
                        groups: Object.keys(groupMapping || {}),
                        totalSkus: Object.values(groupMapping || {}).reduce((sum, g) => { var _a; return sum + (((_a = g.skus) === null || _a === void 0 ? void 0 : _a.length) || 0); }, 0),
                    });
                }
                ...
                if (rawJsonAtEnd) {
                    ...
                    logger_1.default.info("📝 [LLMFormatter] Stripped raw groupMapping JSON from response");
                }
                ...
                if (genericJsonEnd) {
                    ...
                    logger_1.default.info("📝 [LLMFormatter] Stripped generic JSON mapping from end of response");
                }
                // Clean up any formatting artifacts
                text = text.replace(/---BEGIN OUTPUT---\n?/g, "").replace(/\n?---END OUTPUT---/g, "").trim();
                logger_1.default.info("📝 [LLMFormatter] Formatted response", {
                    type: response.type,
                    tokensUsed,
                    hasGroupMapping: !!groupMapping,
                    groupMappingSource: jsonMatch ? "LLM" : (((_f = response.data) === null || _f === void 0 ? void 0 : _f.groupMapping) ? "CODE" : "NONE"),
                    ms: Date.now() - startTime,
                });
                return {
                    text,
                ...
                logger_1.default.error("❌ [LLMFormatter] Error calling LLM", { error });
    ...
    // ================================================================================
    tryTemplateResponse(response, targetLanguage) {
...
• "where are you" - address and contacts`,
    ...
    getProductDetailTemplate(response, targetLanguage) {
        ...
        const codeAndFormat = [];
        if (product.sku)
            codeAndFormat.push(`Codice: ${product.sku}`);
        if (product.formato)
            codeAndFormat.push(`Formato: ${product.formato}`);
    ...
    getCartViewTemplate(response) {
        ...
        const products = items.filter((item) => item.itemType === "PRODUCT" || item.type === "PRODUCT" || !item.itemType);
        const services = items.filter((item) => item.itemType === "SERVICE" || item.type === "SERVICE");
        // Display Products
        ...
        const grandTotal = cart.totalAmount + ((_b = (_a = cart.transport) === null || _a === void 0 ? void 0 : _a.totalTransportCost) !== null && _b !== void 0 ? _b : 0);
        lines.push("");
        ...
        if (response.context.hasDiscount && response.context.discountPercent > 0) {
            lines.push("");
            lines.push(`ℹ️ Stai usufruendo del tuo sconto riservato del <b>${response.context.discountPercent}</b>%! I prezzi mostrati includono già lo sconto.`);
    ...
    // PROMPT BUILDING
    // ================================================================================
    buildFormattingPrompt(response, targetLanguage) {
        const parts = [];
    ...
    formatCategoryListPrompt(response) {
        const items = response.data.items || [];
    ...
    formatProductListPrompt(response) {
        const items = response.data.items || [];
    ...
    formatProductGroupedPrompt(response) {
    ...
    formatCatalogAggregatePrompt(response) {
        const aggregate = response.data.aggregateResult;
        if (!aggregate) {
            return "Aggregate result not available.";
        }
        const typeLabel = aggregate.type === "min"
            ? "valore minimo"
            : aggregate.type === "max"
                ? "valore massimo"
                : "conteggio totale";
        const valueLabel = aggregate.field === "price" && aggregate.type !== "count"
     ...
     */
    formatSmartGroupingPrompt(response) {
        const items = response.data.items || [];
        ...
        const productList = items.map((item) => {
            ...
            if (item.description)
                line += ` - ${item.description.substring(0, 80)}`;
            return line;
        }).join("\n");
...
- Include ALL SKUs, each product must appear in only one group`;
    ...
    formatServiceListPrompt(response) {
        const items = response.data.items || [];
    ...
    formatServiceDetailPrompt(response) {
...
- DO NOT show service codes to the user`);
    ...
    formatProductDetailPrompt(response) {
        var _a;
        const p = response.data.product;
        if (!p)
            return "Product not found";
        const displayPrice = p.priceWithDiscount || p.price;
        const targetLang = ((_a = response.context.customerLanguage) === null || _a === void 0 ? void 0 : _a.toLowerCase()) || "";
    ...
    formatCartPrompt(response) {
        ...
        const resolveQuantity = (item) => {
            ...
            if (typeof item.extra === "string") {
                ...
                if (digits === null || digits === void 0 ? void 0 : digits[0]) {
        ...
        const resolveLineTotal = (item, quantity) => {
        ...
        if (response.context.hasDiscount && response.context.discountPercent && response.context.discountPercent > 0) {
            outputLines.push("");
            outputLines.push(`ℹ️ Stai usufruendo del tuo sconto riservato del <b>${response.context.discountPercent}</b>%! I prezzi mostrati includono già lo sconto.`);
    ...
    formatOrderListPrompt(response) {
        const items = response.data.items || [];
        const lines = [
            "FORMAT RULES:",
    ...
    formatOrderDetailPrompt(response) {
     ...
     */
    formatBusinessInfoPrompt(response) {
     ...
     */
    formatFAQPrompt(response) {
        const data = response.data;
        const faqs = (data === null || data === void 0 ? void 0 : data.faqs) || [];
     ...
     */
    formatOfferWithProductsPrompt(response) {
        var _a;
        ...
        for (const item of items) {
            ...
            if (product) {
                ...
                lines.push(`**${item.number}.** ${item.name} - €${((_a = item.price) === null || _a === void 0 ? void 0 : _a.toFixed(2)) || 'N/A'} [SKU:${item.sku}]`);
            }
        }
        lines.push("");
        lines.push("INSTRUCTIONS:");
     ...
     */
    formatAgentInfoPrompt(response) {
    ...
    // ================================================================================
    fallbackFormat(response, targetLanguage) {
        switch (response.type) {
            case "CATEGORY_LIST":
    ...
    fallbackCart(response) {
        ...
        const lines = ["Ecco il tuo carrello:", ""];
        // Separate products from services
        const products = cart.items.filter((item) => item.itemType === "PRODUCT" || item.type === "PRODUCT" || (!item.itemType && !item.serviceCode));
        const services = cart.items.filter((item) => item.itemType === "SERVICE" || item.type === "SERVICE" || item.serviceCode);
        // Display Products
        if (products.length > 0) {
            lines.push("🛒 Prodotti:");
            for (const item of products) {
        ...
        if (services.length > 0) {
            ...
            lines.push("🔧 Servizi:");
            for (const item of services) {
                lines.push(`- ${item.productName || item.serviceName || "Servizio"} - ${formatDisplayPrice(item.totalPrice)}`);
            }
        }
        lines.push("");
    ...
    buildGroupMappingData(response) {
        var _a;
        const existing = (_a = response.data) === null || _a === void 0 ? void 0 : _a.groupMapping;
    ...
    buildGroupMappingBlock(response) {
        const data = this.buildGroupMappingData(response);
        return JSON.stringify(data || {});
    }
}
exports.LLMFormatterService = LLMFormatterService;
// ================================================================================
// SINGLETON INSTANCE
// ================================================================================
let formatterInstance = null;
function getLLMFormatter(prisma) {
    if (!formatterInstance) {
        formatterInstance = new LLMFormatterService(prisma);
    }
    return formatterInstance;
}
//# sourceMappingURL=llm-formatter.service.js.map