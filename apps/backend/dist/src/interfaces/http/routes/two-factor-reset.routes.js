 ...
 *                   type: string
 */
router.get("/2fa-reset/:token", (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const { token } = req.params;
        const ipAddress = req.ip || req.socket.remoteAddress;
        const result = yield twoFactorResetService.validateToken(token, ipAddress);
        if (!result.valid) {
        ...
        logger_1.default.error("Error validating 2FA reset token:", error);
        res.status(500).json({
            valid: false,
 ...
 */
router.post("/2fa-reset/:token/start", (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const { token } = req.params;
        const ipAddress = req.ip || req.socket.remoteAddress;
        // Validate token
        ...
        const resetToken = yield database_1.prisma.twoFactorResetToken.findFirst({
            where: { token },
            include: { user: true },
        });
        if (!resetToken) {
        ...
        logger_1.default.info(`2FA reset started for user ${maskedEmail} (token: ${token.substring(0, 8)}...)`);
        res.json({
            success: true,
            email: maskedEmail,
        ...
        logger_1.default.error("Error starting 2FA reset:", error);
        res.status(500).json({
            success: false,
 ...
 */
router.post("/2fa-reset/:token/verify-password", (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const { token } = req.params;
        ...
        const result = yield twoFactorResetService.verifyPassword(token, email, password);
        if (!result.success) {
            return res.status(400).json({
                success: false,
                error: result.error,
        ...
        logger_1.default.error("Error verifying password for 2FA reset:", error);
        res.status(500).json({
            success: false,
 ...
 */
router.post("/2fa-reset/:token/complete", (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const { token } = req.params;
        ...
        yield database_1.prisma.$transaction([
            ...
            // Mark token as used
            database_1.prisma.twoFactorResetToken.update({
                where: { id: resetToken.id },
                data: { usedAt: new Date() },
            }),
        ]);
        logger_1.default.info(`2FA reset completed for user ${resetToken.user.email}`);
        res.json({
            success: true,
        ...
        logger_1.default.error("Error completing 2FA reset:", error);
        res.status(500).json({
            success: false,
...
//# sourceMappingURL=two-factor-reset.routes.js.map