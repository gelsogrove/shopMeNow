...
exports.softDeleteCleanupJob = softDeleteCleanupJob;
...
function getRetentionDays() {
    ...
    if (isNaN(parsed) || parsed < 1) {
        logger_1.default.warn(`Invalid SOFT_DELETE_RETENTION_DAYS value "${envValue}", using default ${DEFAULT_RETENTION_DAYS}`);
        return DEFAULT_RETENTION_DAYS;
    }
    return parsed;
}
async function softDeleteCleanupJob() {
    ...
    if (userIds.length === 0 && workspaceIds.length === 0) {
        logger_1.default.info(`No expired soft-deleted records to hard-delete`);
        return;
    }
    logger_1.default.info(`Starting hard-delete: ${userIds.length} users, ${workspaceIds.length} workspaces`);
    ...
    const totalDeleted = Object.values(deletedCounts).reduce((sum, count) => sum + count, 0);
    logger_1.default.info(`Hard-deleted ${totalDeleted} expired soft-deleted records (retention: ${retentionDays} days, took ${duration}ms)`, {
...
//# sourceMappingURL=soft-delete-cleanup.job.js.map