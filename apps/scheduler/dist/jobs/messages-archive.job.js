...
exports.messagesArchiveJob = messagesArchiveJob;
...
async function messagesArchiveJob() {
    const cutoffDate = new Date();
    cutoffDate.setMonth(cutoffDate.getMonth() - ARCHIVE_AFTER_MONTHS);
    logger_1.default.info(`📦 Starting messages archive job (archiving before ${cutoffDate.toISOString()})`);
    let totalArchived = 0;
    let hasMore = true;
    while (hasMore) {
        // Find old messages with their session info (for denormalization)
        const oldMessages = await database_1.prisma.message.findMany({
            where: {
                createdAt: {
                    lt: cutoffDate,
                },
        ...
        logger_1.default.info(`📦 Archived batch: ${oldMessages.length} messages (total: ${totalArchived})`);
    ...
    if (totalArchived > 0) {
        logger_1.default.info(`✅ Messages archive completed: ${totalArchived} messages archived`);
    }
    else {
        logger_1.default.info(`✅ No messages to archive (all messages are newer than ${ARCHIVE_AFTER_MONTHS} months)`);
    }
}
//# sourceMappingURL=messages-archive.job.js.map