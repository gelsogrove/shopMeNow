...
exports.setupOrphanedFilesCleanup = setupOrphanedFilesCleanup;
exports.setupTempFilesCleanup = setupTempFilesCleanup;
exports.setupInvoiceCleanup = setupInvoiceCleanup;
exports.setupStorageCleanup = setupStorageCleanup;
 ...
 */
function setupOrphanedFilesCleanup() {
    node_cron_1.default.schedule('0 3 * * *', async () => {
        ...
        try {
            ...
            const activeProducts = await database_1.prisma.products.findMany({
            ...
            const activeServices = await database_1.prisma.services.findMany({
                select: { imageKey: true }
            });
            ...
            const activeWorkspaces = await database_1.prisma.workspace.findMany({
                select: { logoKey: true }
            });
 ...
 * Runs every hour
 */
function setupTempFilesCleanup() {
 ...
 */
function setupInvoiceCleanup() {
    node_cron_1.default.schedule('0 4 * * *', async () => {
        console.log('🧹 Starting invoice cleanup...');
        try {
            const storage = getStorageService();
            const cancelledOrders = await database_1.prisma.orders.findMany({
                where: {
                    status: 'CANCELLED',
                    invoiceKey: { not: null }
                }
            ...
            for (const order of cancelledOrders) {
                if (order.invoiceKey) {
                    ...
                    await database_1.prisma.orders.update({
                        where: { id: order.id },
                        data: {
                            invoiceUrl: null,
                            invoiceKey: null
