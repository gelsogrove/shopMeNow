...
exports.whatsappChallengeQueueJob = whatsappChallengeQueueJob;
const database_1 = require("../config/database");
const logger_1 = __importDefault(require("../utils/logger"));
const security_agent_service_1 = require("../services/security-agent.service");
 ...
 */
async function appendTimelineStep(conversationMessageId, step) {
    if (!conversationMessageId) {
        logger_1.default.debug('[WhatsApp Queue] No conversationMessageId - skipping timeline append');
        return;
    }
    try {
        const message = await database_1.prisma.conversationMessage.findUnique({
            where: { id: conversationMessageId },
            select: { id: true, debugInfo: true },
        });
        if (!message) {
            logger_1.default.warn(`[WhatsApp Queue] Conversation message ${conversationMessageId} not found for timeline append`);
            return;
        }
        ...
        await database_1.prisma.conversationMessage.update({
            where: { id: conversationMessageId },
            data: { debugInfo: JSON.stringify(debugInfo) },
        });
        logger_1.default.debug(`[WhatsApp Queue] Appended "${step.agent}" step to timeline for message ${conversationMessageId}`);
    }
    catch (error) {
        logger_1.default.error(`[WhatsApp Queue] Error appending timeline step:`, error);
    }
}
/**
 ...
 */
async function whatsappChallengeQueueJob() {
    // 🔒 LOCK CHECK: Skip if previous job is still running
    if (isProcessing) {
        logger_1.default.debug('[WhatsApp Queue] Skipping - previous job still running');
        return;
    }
    isProcessing = true;
    const startTime = Date.now();
    try {
        const securityAgent = new security_agent_service_1.SecurityAgentService();
        ...
        if (workspaces.length === 0) {
            logger_1.default.debug('[WhatsApp Queue] No workspaces with active channel found');
            return;
        }
        ...
        for (const workspace of workspaces) {
            ...
            if (workspace.debugMode === true) {
                logger_1.default.debug(`[WhatsApp Queue] 🔧 DEBUG MODE for "${workspace.name}" - messages stay pending`);
                continue;
            }
            // Get pending messages from queue
            // ⚠️ LIMIT TO 10 per workspace per cycle
            const pendingMessages = await database_1.prisma.whatsAppQueue.findMany({
                where: {
            ...
            if (pendingMessages.length === 0)
                continue;
            logger_1.default.info(`[WhatsApp Queue] Processing ${pendingMessages.length} messages for workspace: ${workspace.name}`);
            // 🚀 PARALLEL PROCESSING: Send all messages simultaneously
            // Safe because WhatsApp limit is per-pair (same customer), not global
            const results = await Promise.allSettled(pendingMessages.map(async (message) => {
                try {
                    ...
                    if (!securityCheck.isSafe) {
                        ...
                        await database_1.prisma.whatsAppQueue.update({
                            where: { id: message.id },
                            data: {
                                status: 'blocked',
                    ...
                    try {
                        ...
                        const deductResult = await billingService.deductMessageCredit(workspace.id, message.id);
                        if (deductResult.success) {
                            logger_1.default.info(`[WhatsApp Queue] 💰 Credit deducted for message ${message.id}: €${deductResult.amountDeducted?.toFixed(2)} → Balance: €${deductResult.newBalance?.toFixed(2)}`);
                        }
                        else {
                            logger_1.default.warn(`[WhatsApp Queue] ⚠️ Failed to deduct credit: ${deductResult.error}`);
                        }
                    }
                    catch (billingError) {
                        logger_1.default.error(`[WhatsApp Queue] ⚠️ Billing error for message ${message.id}:`, billingError);
                    ...
                    return { status: 'error', messageId: message.id, error: error.message };
                }
            }));
            // Count results
        ...
        if (totalProcessed > 0 || totalBlocked > 0 || totalErrors > 0) {
            logger_1.default.info(`[WhatsApp Queue] ✅ Cycle completed in ${duration}ms: ${totalProcessed} sent, ${totalBlocked} blocked, ${totalErrors} errors`);
        }
    }
    catch (error) {
        logger_1.default.error('[WhatsApp Queue] ❌ Job error:', error);
    }
    finally {
...
//# sourceMappingURL=whatsapp-challenge-queue.job.js.map