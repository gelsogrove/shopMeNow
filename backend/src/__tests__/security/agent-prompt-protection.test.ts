...
describe('🔒 CRITICAL SECURITY: Agent Prompt Protection', () => {
  describe('Admin-Only Prompt Modification', () => {
    it('should ALLOW admin to update prompt', async () => {
      ...
      const agentService = {
        updateAgentConfig: async (id: string, data: any, workspaceId: string, userId?: string) => {
  ...
  describe('Logging and Audit Trail', () => {
    ...
    it('should log blocked non-admin attempts', async () => {
      ...
      const agentService = {
        updateAgentConfig: async (id: string, data: any, workspaceId: string, userId?: string) => {
          if (userId && (data.prompt || data.content)) {
            const user = await mockPrisma.user.findUnique({ where: { id: userId } })
            if (!user || user.role !== 'ADMIN') {
              mockLogger.warn(`🚨 SECURITY: Non-admin user ${userId} attempted to modify agent prompt`)
              throw new Error('Only admin users can modify agent prompts')
            }
          }
        },
      ...
      expect(logs).toContain('🚨 SECURITY: Non-admin user user-regular attempted to modify agent prompt')
    })
  })
})
