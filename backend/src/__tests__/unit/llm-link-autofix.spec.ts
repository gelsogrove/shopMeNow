...
describe("LLM Link Auto-Fix", () => {
  ...
  beforeEach(() => {
    ...
    mockCallingFunctionsService = (llmService as any)
      .callingFunctionsService as jest.Mocked<CallingFunctionsService>
  })
  ...
  describe("Auto-Fix Hardcoded /orders Links", () => {
    it("should detect and replace hardcoded /orders link", async () => {
      ...
      const llmResponse =
        "Ecco il link per vedere tutti i tuoi ordini: http://localhost:3000/orders ⏰ Link valido per 1 ora."
      ...
      const autoFixReplacement = linkReplacements.find(
        (r) => r.autoFixed === true
      )
      expect(autoFixReplacement).toBeDefined()
      expect(autoFixReplacement?.token).toContain("AUTO-FIX")
      expect(autoFixReplacement?.replacedWith).toBe(
      ...
      expect(
        mockCallingFunctionsService.getOrdersListLink
    ...
    it("should NOT replace /orders-public links", async () => {
      const llmResponse =
        "Puoi vedere l'ordine qui: http://localhost:3000/orders-public/ORD-001?token=xyz"
      ...
      expect(result).toContain(
        "http://localhost:3000/orders-public/ORD-001?token=xyz"
      )
    ...
    it("should replace multiple hardcoded /orders links", async () => {
      ...
      expect(result).not.toContain("http://localhost:3000/orders")
      expect(
        (result.match(/http:\/\/localhost:3000\/s\/abc123/g) || []).length
      ).toBe(2)
    })
  })
  ...
  describe("Auto-Fix Hardcoded /checkout Links", () => {
    it("should detect and replace hardcoded /checkout link", async () => {
      ...
      const llmResponse =
        "Clicca qui per il carrello: http://localhost:3000/checkout"
  ...
  describe("Token Replacement (Normal Flow)", () => {
    it("should replace [LINK_ORDERS_WITH_TOKEN] with proper link", async () => {
      ...
      const llmResponse =
        "Ecco il link: [LINK_ORDERS_WITH_TOKEN] ⏰ Link valido per 1 ora."
      ...
      const normalReplacement = linkReplacements.find(
        (r) => r.token === "[LINK_ORDERS_WITH_TOKEN]"
      )
      expect(normalReplacement).toBeDefined()
      expect(normalReplacement?.autoFixed).toBeUndefined() // NOT an auto-fix
      expect(normalReplacement?.replacedWith).toBe(
        "http://localhost:3000/s/def456"
      )
    })
    ...
    it("should replace [LINK_CHECKOUT_WITH_TOKEN] with proper link", async () => {
      ...
      const normalReplacement = linkReplacements.find(
        (r) => r.token === "[LINK_CHECKOUT_WITH_TOKEN]"
      )
      expect(normalReplacement).toBeDefined()
