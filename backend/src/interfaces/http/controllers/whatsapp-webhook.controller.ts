...
import { whatsAppToMarkdown, markdownToWhatsApp } from '../../../utils/whatsapp-formatter'
...
export class WhatsAppWebhookController {
  ...
  async receiveMessage(req: Request, res: Response): Promise<void> {
    try {
      ...
      if (!isValidSignature) {
        logger.error('[WEBHOOK] ❌ Invalid HMAC signature - potential attack!', {
      ...
      if (!customer) {
        logger.warn('[WEBHOOK] ⚠️ Customer not found for phone', { phoneNumber })
        
        // Send "please register" message
        const registerMessage = 'Hello! Please register first to use our service. Visit our website or contact support.'
      ...
      if (!customer.workspace.whatsappApiKey || !customer.workspace.whatsappPhoneNumber) {
