...
export async function sendWhatsAppNotification(
  ...
  try {
    ...
    if (!customer) {
      logger.error("[WHATSAPP-NOTIFICATION] ❌ Customer not found", {
        customerId,
      })
      return {
        success: false,
    ...
    if (customer.workspaceId !== workspaceId) {
      logger.error(
        "[WHATSAPP-NOTIFICATION] ❌ Customer does not belong to workspace",
        {
          customerId,
    ...
    if (!workspace?.whatsappApiKey || !workspace?.whatsappPhoneNumber) {
      logger.error(
        "[WHATSAPP-NOTIFICATION] ❌ WhatsApp not configured for workspace",
        {
          workspaceId,
