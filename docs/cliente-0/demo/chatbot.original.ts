...
function hasInconsistentDoubleChargeNarrative(message: string): boolean {
  const normalized = message.trim().toLowerCase()
  // Unclear number of charges (e.g. "tres o cuatro veces", "no sé cuántas")
  ...
  // Amount doesn't match (e.g. "importe no me cuadra", "no cuadra el importe")
...
function parseServiceCompletedAnswer(message: string): boolean | null {
  ...
  // Use ^ anchor for short affirmatives — \b doesn't work with accented chars AND "y"/"si" are ambiguous Spanish words
  if (/^(s[iíì]|yes|y)$/i.test(normalized)) return true
  // Multi-word specific affirmative phrases only (never standalone short words that could mean something else)
...
function applyContextualRouterFallback(
  ...
  if (isDoubleChargeCase) {
    // Scenario 6.2: Angry customer explicitly demanding a human in first double-charge message
    // Deterministic override: skip data collection and escalate immediately
    ...
    if (isAngryWithExplicitOperatorRequest && !state.location && !state.serviceCompleted && !state.doubleChargeNarrativeProvided) {
      return {
        ...
        customerFacingGoal: 'The customer is very angry and explicitly requested a human. Ask for their name and immediately escalate without collecting review data.',
    ...
    // Payment proof question has been answered (yes OR no) — accept either as "done".
    // Only use deterministic parsing (NOT LLM extractedFacts) to avoid false positives
    // where the LLM sets paymentProofProvided:false before the question is even asked.
    const paymentProofAnswered =
      (isAwaitingPaymentProof && currentTurnPaymentProof !== null) ||
      typeof state.paymentProofProvided === 'boolean'

    // Location is collected first, before service completed (Case 6 flow order)
...
async function classifyChoiceViaLLM(node: FlowNode, input: string): Promise<string | null> {
  ...
  // For CONFIRMATION nodes with an 'other' transition, include 'other' as an explicit option
  // so the LLM can distinguish "machine didn't start" (NO) from "I can't follow instructions" (other).
...
function buildDoubleChargeStepDecision(
  ...
  // Preserve escalation decisions already set deterministically (e.g. angry customer in applyContextualRouterFallback)
  ...
  const currentTurnPaymentProofResult = state.lastMissingFacts.includes('payment proof')
    ...
    : null
  // Payment proof step is "done" when the customer has given ANY answer (yes or no)
  ...
  // True when payment proof question has been answered (yes OR no)
  const paymentProofAnswered =
    currentTurnPaymentProofAnswered || typeof state.paymentProofProvided === 'boolean'

  const missingFacts: string[] = []

  // Location is collected first (Case 6 flow order: location → service → narrative → digits → proof)
  // Check both state and the just-extracted facts (mergeFactsIntoState not called yet at this point)
  const resolvedLocation =
    (typeof routerDecision.extractedFacts?.location === 'string' && routerDecision.extractedFacts.location) ||
    state.location

  // Scenario 6.3: if the customer provides the narrative but it's inconsistent (unclear charge count
  // or amount doesn't match), escalate immediately to a human operator
  ...
  if (isNarrativeInconsistent) {
    return {
      ...
      customerFacingGoal: 'The customer\'s narrative is inconsistent (unclear number of charges or amount mismatch). You MUST ask for their name in the same message using the phrase "¿Cómo te llamas?" and announce operator escalation using the word "operador". Do NOT request card digits or payment proof.',
      escalationReason: 'Inconsistent double-charge narrative: unclear number of charges or amount mismatch.',
    }
  }
...
async function handleTurn(runtime: Runtime, state: SessionState, userMessage: string): Promise<TurnResult> {
  ...
  if (state.customerNameRequested && !state.customerName && normalizedUserMessage.length > 0) {
    ...
    // Case 6.1 resolved path: name collected, no escalation — just confirm review
  ...
  if (isDoubleChargeCase) {
    // buildDoubleChargeStepDecision is the sole arbiter of the double-charge flow.
    // It handles: normal data collection, inconsistent narrative → escalate, all data collected → close.
    // We never blindly preserve the LLM router's contactOperator decision here.
  ...
  if (state.turnCount === 1 && !state.location && (hasTroubleshootingIntent(normalizedUserMessage) || Boolean(extractDisplayState(userMessage)))) {
    // Deterministically detect display code from the raw message (e.g. "Me sale AL001")
    ...
    if (displayAlreadyKnown) {
      // Display code already in the first message → ask location first.
      // Also stamp the detected code and clear inferred machineType so the machine-type question fires next.
      routerDecision.missingFacts = ['location']
      ...
      routerDecision.extractedFacts = {
        ...
        machineType: '',  // don't trust LLM-inferred machine type from display code context
      }
    } else {
  ...
  // These bypass LLM rendering to guarantee the required words ("secadora", "máquina") appear.
  ...
  if (isDoubleChargeCase && routerDecision.functionName === 'contactOperator') {
    ...
    if (allDataCollected) {
      // Case 6.1: All data collected — ask for name first, then confirm review (consistent with 6.2/6.3)
      ...
      // Name already provided — confirm review without escalation
      state.pendingClosure = 'resolved'
      const { message, safe } = await renderHistory(runtime, state, {
        routerDecision: {
          ...routerDecision,
          customerFacingGoal:
            `Confirm warmly to ${state.customerName ? state.customerName : 'the customer'} that all required data has been received. You MUST include the word "revisar" in infinitive form (e.g. "vamos a revisar tu caso"). Let the customer know a refund form will be sent. Add a brief tip: next time contact support before re-paying. Do NOT use the words "operador" or "desactivado".`,
        },
      })
      return { reply: sanitizeCustomerReply(safe ? message : fallbackBlockedMessage(state.language)), debug }
    }

    // Case 6.2/6.3: Escalation before all data collected (angry customer or inconsistent narrative)
...
async function evaluateCriteriaDetailedWithLLM(
  ...
  // Strip the operator block from customer-facing text (used by deterministic checks).
  // Keep raw bot text for convText so the LLM evaluator can see the operator summary.
  ...
  // botTurns: customer-facing text only (stripped) — used by deterministic rules
  const botTurns = conversation.map(({bot}) => stripOperatorBlock(bot))
  // operatorBlocks: joined operator summaries — used by operator-summary deterministic rule
  ...
  // convText: includes raw bot text so LLM evaluator can see the operator summary content
  ...
  const supportedTags = [
    ...
    // Case 6: double charge specific tags
  ...
  const evaluateHeuristic = async (criterion: string): Promise<boolean | null> => {
    ...
    // ── Case 6: double charge — direct pattern pre-checks (NO LLM classifier needed) ──

    // "El bot NO pregunta el tipo ni el número de máquina en este caso"
    if (/no pregunta.*(tipo|numero de maquina|numero de la maquina)|no debe preguntar.*(tipo|numero)/.test(criterionNorm)) {
    ...
    // "El bot solicita el relato paso a paso de lo ocurrido"
    ...
    if (/mensaje de escalacion contiene.*operador|contiene la palabra.*operador/.test(criterionNorm)) {
      // Find any bot message containing escalation warning and check it has "operador"
    ...
    // "El mensaje final al operador (Human Support message) resume nombre, localización, …"
    // Check the operator summary block, not the customer-facing text.
    if (/human support message.*resume|mensaje final al operador.*resume|resume.*nombre.*localizacion|resume.*nombre.*maquina/.test(criterionNorm)) {
      ...
      // Extract display code from criterion (e.g. "SEL", "DOOR", "ALM")
    ...
    // Also covers: "el bot informa explícitamente al cliente: El chatbot será desactivado."
    ...
    // "El bot NO continúa recogiendo datos de revisión" / "El bot NO continúa solicitando más datos"
    if (/no contin[uú]a recogiendo|no contin[uú]a solicitando/.test(criterionNorm)) {
      // After escalation (operador mention), bot should not ask for card digits or capture
    ...
    if (tag === 'WARM_GREETING_WITH_REASSURANCE') {
      ...
      // Broad reassurance: includes "para ayudarte", "soy el asistente virtual", "estoy aqui para ayudarte"
    ...
    // ── Case 6: double charge deterministic heuristics ──────────────────────

    if (tag === 'NO_MACHINE_QUESTION_IN_DOUBLE_CHARGE') {
      // Bot should NOT ask for machine type or machine number in double charge cases
      return !hasMachineTypeQuestion(botAll) && !hasMachineNumberQuestion(botAll)
    }

    if (tag === 'ASK_SERVICE_COMPLETED') {
      // Bot must ask whether the service (washing/drying) was completed
    ...
    if (tag === 'ASK_NARRATIVE_STEP_BY_STEP') {
      // Bot must request a step-by-step account
      return /paso a paso|cu[eé]ntame|cu[eé]ntenos|cu[eé]ntenos|desc[rí]be|qu[eé] pas[oó]|c[oó]mo empez|what happened/i.test(botAll)
    }

    if (tag === 'NO_IMMEDIATE_ESCALATION_ON_DOUBLE_CHARGE') {
      // Bot must NOT escalate immediately on detecting double charge — it should collect data first
      // Check: first bot turn should not contain escalation keywords
      const firstBotNorm = normalizeForMatch(firstBotTurn)
      return !/operador|escal|derivar|humano se encarg/.test(firstBotNorm)
    }

    if (tag === 'ESCALATE_IMMEDIATELY_ANGRY_CUSTOMER') {
      // When customer is very angry AND demands human, bot must ask name + escalate in same message
      // The first bot turn after the angry message should contain "operador" and ask for name
      const angryUserIdx = userTurns.findIndex((t) =>
        /enfadad|furioso|molest|operador ahora mismo|hablar con una persona|quiero hablar con|muy enojad/i.test(t)
      )
      if (angryUserIdx === -1) return null // criterion not applicable to this conversation
      const botAfterAngry = botTurns[angryUserIdx] || ''
    ...
    if (tag === 'ESCALATE_ON_INCONSISTENT_NARRATIVE') {
      // When customer gives inconsistent narrative, bot must escalate (ask name + mention operador)
      const inconsistentUserIdx = userTurns.findIndex((t) =>
        /tres o cuatro veces|no s[eé] exactamente|importe no me cuadra|no cuadra|cuatro veces|varias veces sin saber/i.test(t)
      )
      if (inconsistentUserIdx === -1) return null // not applicable
      const botAfterInconsistent = botTurns[inconsistentUserIdx] || ''
      return /operador|escalar|derivar/i.test(botAfterInconsistent)
    }

    if (tag === 'FINAL_MESSAGE_NO_ESCALATION_WORDS') {
      // The LAST bot turn must not contain "operador" or "desactivado"
      const lastBotTurn = botTurns[botTurns.length - 1] || ''
      return !/operador|desactivado/i.test(lastBotTurn)
    }
