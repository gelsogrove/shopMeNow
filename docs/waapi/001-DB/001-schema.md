...
ALTER TABLE workspaces
  ...
  ADD COLUMN waapiQrCodeUpdatedAt TIMESTAMP(3), -- Added for TTL cleanup
...
- `waapiQrCodeData` is temporary; should be cleaned up by TTL job checking `waapiQrCodeUpdatedAt`.
...
2. `waapiQrCodeUpdatedAt` column exists.
3. `waapiPhoneNumber` is required for WaAPI onboarding.
4. No existing data is invalidated by migrations.
