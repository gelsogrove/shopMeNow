...
html: `<p>Welcome to <strong>eChatbot</strong>!</p>`
...
<body>
  <table>
    ...
    <tr>
      <td>
        <p>© 2025 eChatbot. ${t.welcome.rights}</p>
...
`
    ...
    const mailOptions = {
      from: `"eChatbot" <${process.env.SMTP_FROM}>`,
      to: data.to,
...
`
    ...
    const mailOptions = {
      from: `"eChatbot Support" <${process.env.SMTP_FROM}>`,
      to: data.to,
