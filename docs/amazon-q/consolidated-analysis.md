...
## � AGGIORNAMENTO CRITICO - 11 Gennaio 2026

**Problemi Risolti**:
- ✅ Due Sistemi Billing: CHIARITO - NON sono duplicati, sono architetture complementari diverse
  - BillingService = Traccia ordini chat LLM (scaricano dal contatore crediti)
  - SubscriptionBillingService = Subscription mensile owner + gestione crediti
- ✅ Soglia Credito: Allineata a -10 su FE e BE- ✅ **Variabili Prompt Duplicati**: FIXATO - Aggiunta validazione rigorosa su `{{products}}`, `{{offers}}`, etc. che impedisce 2+ occorrenze (causa token explosion 100k+)
  - Fix: `validatePromptForDuplicateVariables()` in PromptProcessorService
  - Implementato in: `AgentService.updateAgentConfig()` prima del save
- ✅ **Soft Delete Canali nei Limiti**: RISOLTO - Workspace soft-deleted già esclusi dalla conta dei limiti tramite `deletedAt: null`
**Problemi Critici Rimasti (PRIORITÀ)**:
1. � **Race Condition ChatSession** - NON REALE (coda messaggi sequenziale)
2. 🟢 **Variabili Prompt Duplicati** - RISOLTO (validazione implementata)
3. 🟢 **Soft Delete Canali nei Limiti** - RISOLTO (già esclusi da `deletedAt: null`)
4. 🟠 **Blocco PAYMENT_FAILED Manuale** - Andrea gestisce manualmente (OK per ora)

---

## 📊 Stato Attuale (Aggiornato)

### Completamento Generale: **85%** (inalterato)
...
### 1. Due Sistemi Billing (RISOLTO - Architetture Complementari)
**CHIARITO**: Non sono duplicati, sono DUE SISTEMI DIVERSI E COMPLEMENTARI
- **BillingService**: Traccia costi degli ordini creati da chat LLM (ordini conversazionali)
  - Cliente chattessa: "voglio 2 pizze" → LLM crea ordine → BillingService registra il costo
  - Deducibile dai crediti del proprietario workspace
  - Usa `Billing` table per tracking ordini
- **SubscriptionBillingService**: Gestisce subscription mensile del proprietario + crediti
  - Owner sceglie piano (BASIC/PREMIUM/ENTERPRISE)
  - Subscription mensile su User.creditBalance
  - Crediti si scaricano quando clienti fanno ordini chat (via BillingService)
  - Crediti si ricaricano ogni mese secondo il piano scelto
  - Usa `TransactionHistory` + `User.creditBalance` per tracking
- **Architettura**: Complementari, NON conflittuali ✅
- **Status**: RISOLTO - Sono due domini di business diversi

### 2. Soglia Credito Incoerente (RISOLTO ✅)
**Status**: Backend e Frontend allineati a `-10` ✅
