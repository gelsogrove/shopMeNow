  ...
  private setupHandlers() {
    this.io.on('connection', (socket) => {
      ...
      socket.on('subscribe_workspace', (workspaceId: string) => {
        ...
        logger.info(`Client ${socket.id} subscribed to workspace ${workspaceId}`)
      })
      
      // Unsubscribe from workspace
      socket.on('unsubscribe_workspace', (workspaceId: string) => {
        socket.leave(`workspace:${workspaceId}`)
        logger.info(`Client ${socket.id} unsubscribed from workspace ${workspaceId}`)
      })
...
*Prossimo step: Aspetto tua decisione su quale approccio seguire!*
