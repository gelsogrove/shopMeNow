# demobeauty-chatbot Specification

## Purpose

Defines the conversational behavior of the custom-demobeauty chatbot: a multi-sede beauty-center franchise assistant that handles welcome and sede selection, per-sede service catalogs and pricing, multi-service booking with duration calculation, an in-memory cart for services and products, slot availability enforcement, booking data collection, confirmation emails, Google Calendar event creation, push notifications, operator escalation, multilingual and audio-reciprocal responses, media handling, and cross-sede routing.
