...
describe("BillingSection", () => {
  ...
  // ===========================================================================
  // PLAN LIMITS DISPLAY TESTS
  // ===========================================================================

  describe("Usage Limits Display", () => {
    it("should display products usage correctly (10/50)", async () => {
    ...
    it("should display customers usage correctly (25/50)", async () => {
      render(<BillingSection />)
    ...
    it("should display channels usage correctly (1/1)", async () => {
      render(<BillingSection />)

      await waitFor(() => {
        expect(screen.getByText("WhatsApp Channels")).toBeInTheDocument()
        expect(screen.getByText("1/1")).toBeInTheDocument()
      })
    })

    it("should show warning when channels limit is reached (100%)", async () => {
      render(<BillingSection />)

      await waitFor(() => {
        expect(
          screen.getByText("⚠️ Limit reached - upgrade to add more channels")
        ).toBeInTheDocument()
      })
    })

    it("should show warning when approaching products limit (90%+)", async () => {
      vi.mocked(useBilling).mockReturnValue({
        billingOverview: {
          ...mockBillingOverview,
          usage: {
    ...
    it("should show limit reached message at 100% products", async () => {
      vi.mocked(useBilling).mockReturnValue({
        billingOverview: {
          ...mockBillingOverview,
          usage: {
      ...
      await waitFor(() => {
        expect(
          screen.getByText("⚠️ Limit reached - upgrade to add more")
        ).toBeInTheDocument()
      })
    })
  })
